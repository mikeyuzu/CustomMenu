local magic_definitions = {}

-- 魔法データの定義フォーマット
-- [魔法ID] = { 
--     name = "魔法名", 
--     description = "概要", 
--     shop = "ショップ販売情報",
--     quest = "クエスト報酬情報",
--     drop = "ドロップ情報",
--     steal = "盗む情報",
--     chest = "宝箱情報",
--     mining = "採掘/採取情報",
--     content = "コンテンツ報酬情報",
--     acquisition = "その他入手方法（上記に当てはまらない場合）" 
-- }
-- 習得レベルはAPIから取得するため、ここには定義しません。

magic_definitions.magics = {
    [1] = {
        name = "ケアル",
        description = "標的のHPを回復。ハートオブソラス:ストンスキンの効果を付与する。",
        shop = "バストゥーク商業区 H-5T1 Sororo\nウィンダス港 H-8T8 Kususu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "白魔道士の初期装備",
    },
    [2] = {
        name = "ケアルII",
        description = "標的のHPを回復。ハートオブソラス:ストンスキンの効果を付与する。",
        shop = "バストゥーク商業区 H-5T1 Sororo\nウィンダス港 H-8T8 Kususu\nセルビナ H-9T5 Quelpia",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [3] = {
        name = "ケアルIII",
        description = "標的のHPを回復。ハートオブソラス:ストンスキンの効果を付与する。",
        shop = "ウィンダス水の区 南 G-7T3 Shohrun-Tuhrun\nセルビナ H-9T5 Quelpia",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [4] = {
        name = "ケアルIV",
        description = "標的のHPを回復。ハートオブソラス:ストンスキンの効果を付与する。",
        shop = "ジュノ下層 H-9T9 Hasim\nアルザビ J-8T5 Zafif",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [5] = {
        name = "ケアルV",
        description = "標的のHPを回復。ハートオブソラス:ストンスキンの効果を付与する。",
        shop = "",
        quest = "",
        drop = {
            { zone = "クゥルンの大伽藍", mob = "Ziz" },
            { zone = "オンゾゾの迷宮", mob = "Zeid" },
            { zone = "オズトロヤ城", mob = "Yagudo Sentinel" },
            { zone = "ソ・ジヤ", mob = "Yagudo Sentinel" },
            { zone = "テリガン岬", mob = "Zombie" },
            { zone = "クフタルの洞門", mob = "Zeid" },
            { zone = "グスタフの洞門", mob = "Yagudo's Elemental" },
            { zone = "海蛇の岩窟", mob = "Yilbegan" },
            { zone = "海蛇の岩窟", mob = "Zvahl Fortalice" },
            { zone = "イフリートの釜", mob = "Zombie" },
            { zone = "イフリートの釜", mob = "Zoredonite" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF クローソーオーブ 伝説のチャンピオン\n印章BF ラキシスオーブ 飛竜の眷族\n印章BF アトロポスオーブ 紅蓮の翼\n印章BF クローソーオーブ 蜘蛛の神\n印章BF ラキシスオーブ 破滅の壺\n印章BF アトロポスオーブ 蝿の王",
        acquisition = "",
    },
    [6] = {
        name = "ケアルVI",
        description = "標的のHPを回復。ハートオブソラス:ストンスキンの効果を付与する。",
        shop = "ラバオ F-7T8 Brave Ox",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [7] = {
        name = "ケアルガ",
        description = "範囲内のパーティメンバーのHPを回復。",
        shop = "バストゥーク商業区 H-5T1 Sororo\nウィンダス港 H-8T8 Kususu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [8] = {
        name = "ケアルガII",
        description = "範囲内のパーティメンバーのHPを回復。",
        shop = "ジュノ下層 H-9T9 Hasim\nアルザビ J-8T5 Zafif\nセルビナ H-9T5 Quelpia",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [9] = {
        name = "ケアルガIII",
        description = "範囲内のパーティメンバーのHPを回復。",
        shop = "ジュノ下層 H-9T9 Hasim\nアルザビ J-8T5 Zafif",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [10] = {
        name = "ケアルガIV",
        description = "範囲内のパーティメンバーのHPを回復。",
        shop = "",
        quest = "",
        drop = {
            { zone = "クゥルンの大伽藍", mob = "Ziz" },
            { zone = "オンゾゾの迷宮", mob = "Zeid" },
            { zone = "オズトロヤ城", mob = "Yagudo Sentinel" },
            { zone = "ソ・ジヤ", mob = "Yagudo Sentinel" },
            { zone = "テリガン岬", mob = "Zombie" },
            { zone = "クフタルの洞門", mob = "Zeid" },
            { zone = "グスタフの洞門", mob = "Yagudo's Elemental" },
            { zone = "海蛇の岩窟", mob = "Yilbegan" },
            { zone = "海蛇の岩窟", mob = "Zvahl Fortalice" },
            { zone = "イフリートの釜", mob = "Zombie" },
            { zone = "イフリートの釜", mob = "Zoredonite" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [12] = {
        name = "レイズ",
        description = "標的の戦闘不能状態を回復。",
        shop = "ウィンダス水の区 南 G-7T3 Shohrun-Tuhrun\nセルビナ H-9T5 Quelpia",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [13] = {
        name = "レイズII",
        description = "標的の戦闘不能状態を回復。",
        shop = "",
        quest = "",
        drop = {
            { zone = "クゥルンの大伽藍", mob = "Ziz" },
            { zone = "オズトロヤ城", mob = "Yagudo Sentinel" },
            { zone = "海蛇の岩窟", mob = "Zolku-Azolku" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF コメットオーブ 爆走爬竜\n印章BF コメットオーブ 三つの葛籠\n印章BF ムーンオーブ 第11軍団独立支隊\n印章BF コメットオーブ 虎視眈眈\n印章BF コメットオーブ 戦う羊\n印章BF コメットオーブ 餓鬼武者\n印章BF コメットオーブ 四界の神像\n印章BF ムーンオーブ 潜行特務隊\n印章BF ムーンオーブ 特命介錯人\n印章BF コメットオーブ 刻一刻\n印章BF コメットオーブ 拳闘士の獲物",
        acquisition = "",
    },
    [14] = {
        name = "ポイゾナ",
        description = "標的の毒を治す。",
        shop = "バストゥーク商業区 H-5T1 Sororo\nウィンダス港 H-8T8 Kususu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [15] = {
        name = "パラナ",
        description = "標的の麻痺を治す。",
        shop = "バストゥーク商業区 H-5T1 Sororo\nウィンダス港 H-8T8 Kususu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [16] = {
        name = "ブライナ",
        description = "標的の暗闇を治す。",
        shop = "バストゥーク商業区 H-5T1 Sororo\nウィンダス港 H-8T8 Kususu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [17] = {
        name = "サイレナ",
        description = "標的の静寂、沈黙を治す。",
        shop = "ジュノ下層 H-9T9 Hasim\nアルザビ J-8T5 Zafif",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [18] = {
        name = "ストナ",
        description = "標的の石化を治す。",
        shop = "ジュノ下層 H-9T9 Hasim\nアルザビ J-8T5 Zafif",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [19] = {
        name = "ウィルナ",
        description = "標的の病気、悪疫を治す。",
        shop = "ジュノ下層 H-9T9 Hasim\nアルザビ J-8T5 Zafif",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [20] = {
        name = "カーズナ",
        description = "標的の呪い、呪詛を治す。",
        shop = "ジュノ下層 H-9T9 Hasim\nアルザビ J-8T5 Zafif",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [21] = {
        name = "ホーリー",
        description = "敵に光属性のダメージ。ハートオブソラス:回復量に応じてダメージにボーナスを得る。神聖の印:ダメージにボーナスを得る。",
        shop = "ジュノ下層 H-9T9 Hasim\nアルザビ J-8T5 Zafif\nセルビナ H-9T5 Quelpia",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [23] = {
        name = "ディア",
        description = "敵を光属性のダメージがじわじわ蝕み防御力ダウン。",
        shop = "バストゥーク商業区 H-5T1 Sororo\nウィンダス港 H-8T8 Kususu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "赤魔道士の初期装備",
    },
    [24] = {
        name = "ディアII",
        description = "敵を光属性のダメージがじわじわ蝕み防御力ダウン。",
        shop = "ウィンダス水の区 南 G-7T3 Shohrun-Tuhrun\nセルビナ H-9T5 Quelpia",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [25] = {
        name = "ディアIII",
        description = "敵を光属性のダメージがじわじわ蝕み防御力ダウン。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [28] = {
        name = "バニシュ",
        description = "敵に光属性のダメージ。ハートオブミゼリ:被ダメージ量に応じてダメージにボーナスを得る。神聖の印:ダメージにボーナスを得る。",
        shop = "バストゥーク商業区 H-5T1 Sororo\nウィンダス港 H-8T8 Kususu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [29] = {
        name = "バニシュII",
        description = "敵に光属性のダメージ。ハートオブミゼリ:被ダメージ量に応じてダメージにボーナスを得る。神聖の印:ダメージにボーナスを得る。",
        shop = "ウィンダス水の区 南 G-7T3 Shohrun-Tuhrun\nセルビナ H-9T5 Quelpia",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [30] = {
        name = "バニシュIII",
        description = "敵に光属性のダメージ。ハートオブミゼリ:被ダメージ量に応じてダメージにボーナスを得る。",
        shop = "タブナジア地下壕 2F F-9T8 Mazuro-Oozuro\nタブナジア地下壕 2F F-7T3 Nilerouche\nラバオ F-7T8 Brave Ox",
        quest = "",
        drop = {
            { zone = "海蛇の岩窟", mob = "Zolku-Azolku" },
            { zone = "海蛇の岩窟", mob = "Zvahl Fortalice" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [33] = {
        name = "ディアガ",
        description = "範囲内の敵を光属性のダメージがじわじわ蝕み防御力ダウン。",
        shop = "バストゥーク商業区 H-5T1 Sororo\nウィンダス港 H-8T8 Kususu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [38] = {
        name = "バニシュガ",
        description = "範囲内の敵に光属性のダメージ。ハートオブミゼリ:被ダメージ量に応じてダメージにボーナスを得る。神聖の印:ダメージにボーナスを得る。",
        shop = "バストゥーク商業区 H-5T1 Sororo\nウィンダス港 H-8T8 Kususu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [39] = {
        name = "バニシュガII",
        description = "範囲内の敵に光属性のダメージ。ハートオブミゼリ:被ダメージ量に応じてダメージにボーナスを得る。神聖の印:ダメージにボーナスを得る。",
        shop = "ジュノ下層 H-9T9 Hasim\nアルザビ J-8T5 Zafif",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [43] = {
        name = "プロテス",
        description = "標的の防御力をアップする。",
        shop = "バストゥーク商業区 H-5T1 Sororo\nウィンダス港 H-8T8 Kususu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [44] = {
        name = "プロテスII",
        description = "標的の防御力をアップする。",
        shop = "ウィンダス水の区 南 G-7T3 Shohrun-Tuhrun\nセルビナ H-9T5 Quelpia",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [45] = {
        name = "プロテスIII",
        description = "標的の防御力をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim\nアルザビ J-8T5 Zafif",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [46] = {
        name = "プロテスIV",
        description = "標的の防御力をアップする。",
        shop = "ウィンダス水の区 南 G-7T3 Shohrun-Tuhrun\nアルザビ J-8T5 Zafif\nラバオ F-7T8 Brave Ox",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "戦績BF プークの幻影\n戦績BF 詠唱中は別の顔\n戦績BF オロボンの呪縛",
        acquisition = "",
    },
    [47] = {
        name = "プロテスV",
        description = "標的の防御力をアップする。",
        shop = "ラバオ F-7T8 Brave Ox",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [48] = {
        name = "シェル",
        description = "標的の魔法防御力をアップする。",
        shop = "バストゥーク商業区 H-5T1 Sororo\nウィンダス港 H-8T8 Kususu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [49] = {
        name = "シェルII",
        description = "標的の魔法防御力をアップする。",
        shop = "ウィンダス水の区 南 G-7T3 Shohrun-Tuhrun\nセルビナ H-9T5 Quelpia",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [50] = {
        name = "シェルIII",
        description = "標的の魔法防御力をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [51] = {
        name = "シェルIV",
        description = "標的の魔法防御力をアップする。",
        shop = "",
        quest = "",
        drop = {
            { zone = "クゥルンの大伽藍", mob = "Zoldeff Jagil" },
            { zone = "ガルレージュ要塞", mob = "Zombie" },
            { zone = "デルクフの塔上層", mob = "Ziz" },
            { zone = "ロ・メーヴ", mob = "Ziz" },
            { zone = "ウガレピ寺院", mob = "Yowie" },
            { zone = "ル・オンの庭", mob = "Yagudo Votary" },
            { zone = "ヴェ・ルガノン宮殿", mob = "Ziz" },
            { zone = "ル・アビタウ神殿", mob = "Zazalda Clot" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF クローソーオーブ 伝説のチャンピオン\n印章BF ラキシスオーブ 飛竜の眷族\n印章BF アトロポスオーブ 紅蓮の翼\n印章BF クローソーオーブ 蜘蛛の神\n印章BF ラキシスオーブ 破滅の壺\n印章BF アトロポスオーブ 蝿の王",
        acquisition = "",
    },
    [52] = {
        name = "シェルV",
        description = "標的の魔法防御力をアップする。",
        shop = "ラバオ F-7T8 Brave Ox",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [53] = {
        name = "ブリンク",
        description = "自身の幻影を出して単体攻撃を回避する機会を得る。",
        shop = "バストゥーク商業区 H-5T1 Sororo\nウィンダス港 H-8T8 Kususu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [54] = {
        name = "ストンスキン",
        description = "自身の一定量のダメージを無効にする。",
        shop = "バストゥーク商業区 H-5T1 Sororo\nウィンダス港 H-8T8 Kususu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [55] = {
        name = "アクアベール",
        description = "自身の魔法の中断する確率を下げる。",
        shop = "ウィンダス港 H-8T8 Kususu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [56] = {
        name = "スロウ",
        description = "敵の攻撃間隔を長くする。",
        shop = "バストゥーク商業区 H-5T1 Sororo\nウィンダス港 H-8T8 Kususu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [57] = {
        name = "ヘイスト",
        description = "標的の攻撃間隔を短くする。",
        shop = "ウィンダス水の区 南 G-7T3 Shohrun-Tuhrun\nセルビナ H-9T5 Quelpia",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [58] = {
        name = "パライズ",
        description = "敵を麻痺させる。",
        shop = "",
        quest = "",
        drop = {
            { zone = "龍王ランペールの墓", mob = "Zurko-Bazurko" },
            { zone = "バルクルム砂丘", mob = "Young Quadav" },
            { zone = "コンシュタット高地", mob = "Zoredonite" },
            { zone = "オルデール鍾乳洞", mob = "Zonpa-Zippa" },
            { zone = "ジャグナー森林", mob = "Zo'Dhu Legslicer" },
            { zone = "バタリア丘陵", mob = "Zombie" },
            { zone = "北グスタベルグ", mob = "Zurko-Bazurko" },
            { zone = "北グスタベルグ", mob = "Zuu Xowu the Darksmoke" },
            { zone = "南グスタベルグ", mob = "Zurko-Bazurko" },
            { zone = "パルブロ鉱山", mob = "Ziz" },
            { zone = "パシュハウ沼", mob = "Yowie" },
            { zone = "ロランベリー耕地", mob = "Yearner" },
            { zone = "西サルタバルタ", mob = "Zombie" },
            { zone = "東サルタバルタ", mob = "Zombie" },
            { zone = "ギデアス", mob = "Zu" },
            { zone = "内ホルトト遺跡", mob = "Young Quadav" },
            { zone = "ブブリム半島", mob = "Zymotic Zuulie" },
            { zone = "タロンギ大峡谷", mob = "Ze'Vho Fallsplitter" },
            { zone = "シャクラミの地下迷宮", mob = "Zombie" },
            { zone = "メリファト山地", mob = "Yagudo Scribe" },
            { zone = "ソロムグ原野", mob = "Zurko-Bazurko" },
            { zone = "ラングモント峠", mob = "Ziz" },
            { zone = "デルクフの塔下層", mob = "Zoredonite" },
            { zone = "聖地ジ・タ", mob = "Yhoator Wasp" },
            { zone = "北グスタベルグ〔Ｓ〕", mob = "Zizzy Zillah" },
            { zone = "グロウベルグ〔Ｓ〕", mob = "Za'Dha Adamantking" },
            { zone = "グロウベルグ〔Ｓ〕", mob = "Zeid" },
            { zone = "ロランベリー耕地〔Ｓ〕", mob = "Zo'Bha Helmkeeper" },
            { zone = "クロウラーの巣〔Ｓ〕", mob = "Ziz" },
            { zone = "西サルタバルタ〔Ｓ〕", mob = "Zeid" },
            { zone = "カルゴナルゴ城砦〔Ｓ〕", mob = "Zombie" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [59] = {
        name = "サイレス",
        description = "敵を静寂に捕らえる。",
        shop = "",
        quest = "",
        drop = {
            { zone = "龍王ランペールの墓", mob = "Zurko-Bazurko" },
            { zone = "バルクルム砂丘", mob = "Young Quadav" },
            { zone = "オルデール鍾乳洞", mob = "Zonpa-Zippa" },
            { zone = "ジャグナー森林", mob = "Zo'Dhu Legslicer" },
            { zone = "バタリア丘陵", mob = "Zombie" },
            { zone = "北グスタベルグ", mob = "Zuu Xowu the Darksmoke" },
            { zone = "パシュハウ沼", mob = "Yowie" },
            { zone = "ロランベリー耕地", mob = "Yearner" },
            { zone = "ロランベリー耕地", mob = "Zombie" },
            { zone = "ギデアス", mob = "Yearner" },
            { zone = "内ホルトト遺跡", mob = "Young Quadav" },
            { zone = "ブブリム半島", mob = "Zymotic Zuulie" },
            { zone = "シャクラミの地下迷宮", mob = "Zombie" },
            { zone = "メリファト山地", mob = "Yagudo Scribe" },
            { zone = "メリファト山地", mob = "Zeid" },
            { zone = "ソロムグ原野", mob = "Zonpa-Zippa" },
            { zone = "ソロムグ原野", mob = "Zurko-Bazurko" },
            { zone = "オズトロヤ城", mob = "Zeid" },
            { zone = "ラングモント峠", mob = "Ziz" },
            { zone = "デルクフの塔下層", mob = "Zoredonite" },
            { zone = "聖地ジ・タ", mob = "Yhoator Wasp" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [60] = {
        name = "バファイ",
        description = "自身の火属性に対する耐性をアップする。ハートオブソラス:火属性魔法防御力アップ。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [61] = {
        name = "バブリザ",
        description = "自身の氷属性に対する耐性をアップする。ハートオブソラス:氷属性魔法防御力アップ。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [62] = {
        name = "バエアロ",
        description = "自身の風属性に対する耐性をアップする。ハートオブソラス:風属性魔法防御力アップ。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [63] = {
        name = "バストン",
        description = "自身の土属性に対する耐性をアップする。ハートオブソラス:土属性魔法防御力アップ。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [64] = {
        name = "バサンダ",
        description = "自身の雷属性に対する耐性をアップする。ハートオブソラス:雷属性魔法防御力アップ。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [65] = {
        name = "バウォタ",
        description = "自身の水属性に対する耐性をアップする。ハートオブソラス:水属性魔法防御力アップ。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [66] = {
        name = "バファイラ",
        description = "範囲内にいるパーティメンバーの火属性に対する耐性をアップする。ハートオブソラス:火属性魔法防御力アップ。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [67] = {
        name = "バブリザラ",
        description = "範囲内にいるパーティメンバーの氷属性に対する耐性をアップする。ハートオブソラス:氷属性魔法防御力アップ。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [68] = {
        name = "バエアロラ",
        description = "範囲内にいるパーティメンバーの風属性に対する耐性をアップする。ハートオブソラス:風属性魔法防御力アップ。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [69] = {
        name = "バストンラ",
        description = "範囲内にいるパーティメンバーの土属性に対する耐性をアップする。ハートオブソラス:土属性魔法防御力アップ。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [70] = {
        name = "バサンダラ",
        description = "範囲内にいるパーティメンバーの雷属性に対する耐性をアップする。ハートオブソラス:雷属性魔法防御力アップ。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [71] = {
        name = "バウォタラ",
        description = "範囲内にいるパーティメンバーの水属性に対する耐性をアップする。ハートオブソラス:水属性魔法防御力アップ。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [72] = {
        name = "バスリプル",
        description = "自身の睡眠に対する耐性をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {
            { zone = "龍王ランペールの墓", mob = "Zurko-Bazurko" },
            { zone = "バルクルム砂丘", mob = "Young Quadav" },
            { zone = "コンシュタット高地", mob = "Zoredonite" },
            { zone = "オルデール鍾乳洞", mob = "Zonpa-Zippa" },
            { zone = "ジャグナー森林", mob = "Zo'Dhu Legslicer" },
            { zone = "バタリア丘陵", mob = "Zombie" },
            { zone = "北グスタベルグ", mob = "Zurko-Bazurko" },
            { zone = "北グスタベルグ", mob = "Zuu Xowu the Darksmoke" },
            { zone = "南グスタベルグ", mob = "Zurko-Bazurko" },
            { zone = "パルブロ鉱山", mob = "Ziz" },
            { zone = "パシュハウ沼", mob = "Yowie" },
            { zone = "ロランベリー耕地", mob = "Yearner" },
            { zone = "西サルタバルタ", mob = "Zombie" },
            { zone = "東サルタバルタ", mob = "Zombie" },
            { zone = "ギデアス", mob = "Zu" },
            { zone = "内ホルトト遺跡", mob = "Young Quadav" },
            { zone = "ブブリム半島", mob = "Zymotic Zuulie" },
            { zone = "タロンギ大峡谷", mob = "Ze'Vho Fallsplitter" },
            { zone = "シャクラミの地下迷宮", mob = "Zombie" },
            { zone = "メリファト山地", mob = "Yagudo Scribe" },
            { zone = "ソロムグ原野", mob = "Zurko-Bazurko" },
            { zone = "ラングモント峠", mob = "Ziz" },
            { zone = "デルクフの塔下層", mob = "Zoredonite" },
            { zone = "聖地ジ・タ", mob = "Yhoator Wasp" },
            { zone = "北グスタベルグ〔Ｓ〕", mob = "Zizzy Zillah" },
            { zone = "グロウベルグ〔Ｓ〕", mob = "Za'Dha Adamantking" },
            { zone = "グロウベルグ〔Ｓ〕", mob = "Zeid" },
            { zone = "ロランベリー耕地〔Ｓ〕", mob = "Zo'Bha Helmkeeper" },
            { zone = "クロウラーの巣〔Ｓ〕", mob = "Ziz" },
            { zone = "西サルタバルタ〔Ｓ〕", mob = "Zeid" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [73] = {
        name = "バポイズン",
        description = "自身の毒に対する耐性をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {
            { zone = "龍王ランペールの墓", mob = "Zurko-Bazurko" },
            { zone = "バルクルム砂丘", mob = "Young Quadav" },
            { zone = "オルデール鍾乳洞", mob = "Zonpa-Zippa" },
            { zone = "ジャグナー森林", mob = "Zo'Dhu Legslicer" },
            { zone = "バタリア丘陵", mob = "Zombie" },
            { zone = "北グスタベルグ", mob = "Zuu Xowu the Darksmoke" },
            { zone = "パシュハウ沼", mob = "Yowie" },
            { zone = "ロランベリー耕地", mob = "Yearner" },
            { zone = "ロランベリー耕地", mob = "Zombie" },
            { zone = "内ホルトト遺跡", mob = "Young Quadav" },
            { zone = "ブブリム半島", mob = "Zymotic Zuulie" },
            { zone = "シャクラミの地下迷宮", mob = "Zombie" },
            { zone = "メリファト山地", mob = "Yagudo Scribe" },
            { zone = "ソロムグ原野", mob = "Zurko-Bazurko" },
            { zone = "ラングモント峠", mob = "Ziz" },
            { zone = "デルクフの塔下層", mob = "Zoredonite" },
            { zone = "聖地ジ・タ", mob = "Yhoator Wasp" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [74] = {
        name = "バパライズ",
        description = "自身の麻痺に対する耐性をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [75] = {
        name = "バブライン",
        description = "自身の暗闇に対する耐性をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [76] = {
        name = "バサイレス",
        description = "自身の静寂に対する耐性をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [77] = {
        name = "バブレイク",
        description = "自身の石化に対する耐性をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [78] = {
        name = "バウィルス",
        description = "自身の病気に対する耐性をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [81] = {
        name = "リコールジャグ",
        description = "範囲内の資格があるパーティメンバーをジャグナー森林〔Ｓ〕へワープさせる。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [82] = {
        name = "リコールパシュ",
        description = "範囲内の資格があるパーティメンバーをパシュハウ沼〔Ｓ〕へワープさせる。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [83] = {
        name = "リコールメリファ",
        description = "範囲内の資格があるパーティメンバーをメリファト山地〔Ｓ〕へワープさせる。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [84] = {
        name = "バアムネジア",
        description = "自身のアムネジアに対する耐性をアップする。",
        shop = "マウラ G-9T1 Tya Padolih",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [85] = {
        name = "バアムネジラ",
        description = "範囲内にいるパーティメンバーのアムネジアに対する耐性をアップする。",
        shop = "マウラ G-9T1 Tya Padolih",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [511] = {
        name = "ヘイストII",
        description = "標的の攻撃間隔を短くする。",
        shop = "西アドゥリン ?-?T? Ledericus",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [86] = {
        name = "バスリプラ",
        description = "範囲内にいるパーティメンバーの睡眠に対する耐性をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [87] = {
        name = "バポイゾラ",
        description = "範囲内にいるパーティメンバーの毒に対する耐性をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {
            { zone = "ギデアス", mob = "Yearner" },
            { zone = "メリファト山地", mob = "Zeid" },
            { zone = "ソロムグ原野", mob = "Zonpa-Zippa" },
            { zone = "オズトロヤ城", mob = "Zeid" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [88] = {
        name = "バパライラ",
        description = "範囲内にいるパーティメンバーの麻痺に対する耐性をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [89] = {
        name = "バブライラ",
        description = "範囲内にいるパーティメンバーの暗闇に対する耐性をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [90] = {
        name = "バサイレラ",
        description = "範囲内にいるパーティメンバーの静寂に対する耐性をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [91] = {
        name = "バブレクラ",
        description = "範囲内にいるパーティメンバーの石化に対する耐性をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [92] = {
        name = "バウィルラ",
        description = "範囲内にいるパーティメンバーの病気に対する耐性をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [93] = {
        name = "ケアルラ",
        description = "範囲内のパーティメンバーのHPを回復。ハートオブミゼリ:回復量にボーナスを得る。",
        shop = "ラバオ F-7T8 Brave Ox",
        quest = "",
        drop = {
            { zone = "ベドー", mob = "Zu" },
            { zone = "ユタンガ大森林", mob = "Ziryu" },
            { zone = "海蛇の岩窟", mob = "Ziz" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [94] = {
        name = "サクリファイス",
        description = "標的のパーティメンバーが受けている弱体効果を1つ、自身に移し替える。ハートオブソラス:弱体効果の数や種類にボーナスを得る。",
        shop = "ラバオ F-7T8 Brave Ox",
        quest = "",
        drop = {
            { zone = "オンゾゾの迷宮", mob = "Zeid" },
            { zone = "ソ・ジヤ", mob = "Yagudo Sentinel" },
            { zone = "クフタルの洞門", mob = "Zeid" },
            { zone = "グスタフの洞門", mob = "Yagudo's Elemental" },
            { zone = "イフリートの釜", mob = "Zoredonite" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [95] = {
        name = "エスナ",
        description = "自身が受けている弱体効果を1つ、範囲内のパーティメンバーまで回復。ハートオブミゼリ:弱体効果の数や種類にボーナスを得る。",
        shop = "ラバオ F-7T8 Brave Ox",
        quest = "",
        drop = {
            { zone = "テリガン岬", mob = "Zombie" },
            { zone = "海蛇の岩窟", mob = "Zvahl Fortalice" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [96] = {
        name = "オースピス",
        description = "範囲内のパーティメンバーの与TPを減らす。ハートオブミゼリ:初段のみ光の付加ダメージを与え、且つ攻撃がミスした際に、命中に対してボーナスを得る。",
        shop = "ラバオ F-7T8 Brave Ox",
        quest = "",
        drop = {
            { zone = "ベドー", mob = "Zolku-Azolku" },
            { zone = "オズトロヤ城", mob = "Yearner" },
            { zone = "ズヴァール城内郭", mob = "Zolku-Azolku" },
            { zone = "ズヴァール城内郭", mob = "Zombie" },
            { zone = "海蛇の岩窟", mob = "Zombie" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [310] = {
        name = "エンライト",
        description = "光の付加ダメージを与える。",
        shop = "ナシュモ H-7T6 Mamaroon",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [311] = {
        name = "エンダーク",
        description = "闇の付加ダメージを与える。",
        shop = "ナシュモ H-7T6 Mamaroon",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [100] = {
        name = "エンファイア",
        description = "火の付加ダメージを与える。",
        shop = "ウィンダス水の区 南 G-7T3 Shohrun-Tuhrun\nセルビナ H-9T5 Quelpia\nナシュモ H-7T6 Mamaroon",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [101] = {
        name = "エンブリザド",
        description = "氷の付加ダメージを与える。",
        shop = "ウィンダス水の区 南 G-7T3 Shohrun-Tuhrun\nセルビナ H-9T5 Quelpia\nナシュモ H-7T6 Mamaroon",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [102] = {
        name = "エンエアロ",
        description = "風の付加ダメージを与える。",
        shop = "ウィンダス水の区 南 G-7T3 Shohrun-Tuhrun\nセルビナ H-9T5 Quelpia\nナシュモ H-7T6 Mamaroon",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [103] = {
        name = "エンストーン",
        description = "土の付加ダメージを与える。",
        shop = "ウィンダス水の区 南 G-7T3 Shohrun-Tuhrun\nセルビナ H-9T5 Quelpia\nナシュモ H-7T6 Mamaroon",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [104] = {
        name = "エンサンダー",
        description = "雷の付加ダメージを与える。",
        shop = "ウィンダス水の区 南 G-7T3 Shohrun-Tuhrun\nセルビナ H-9T5 Quelpia\nナシュモ H-7T6 Mamaroon",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [105] = {
        name = "エンウォータ",
        description = "水の付加ダメージを与える。",
        shop = "ウィンダス水の区 南 G-7T3 Shohrun-Tuhrun\nセルビナ H-9T5 Quelpia\nナシュモ H-7T6 Mamaroon",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [106] = {
        name = "ファランクス",
        description = "自身が受けるダメージを一定量軽減する。",
        shop = "",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF スターオーブ 上様御一行\n印章BF スターオーブ 居候妖精\n印章BF ムーンオーブ 第11軍団独立支隊\n印章BF スターオーブ 地獄の尻尾\n印章BF スターオーブ 光る眼\n印章BF スターオーブ 人形工房\n印章BF スターオーブ 生ける屍\n印章BF ムーンオーブ 潜行特務隊\n印章BF ムーンオーブ 特命介錯人\n印章BF スターオーブ 噂のスライム\n印章BF スターオーブ 地竜大王",
        acquisition = "",
    },
    [97] = {
        name = "リアクト",
        description = "魔法の力によって盾の発動率をアップし、防いだダメージの一部を反射する。",
        shop = "アルザビ J-8T5 Zafif",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [108] = {
        name = "リジェネ",
        description = "標的のパーティメンバーのHPを徐々に回復する。",
        shop = "マウラ G-9T1 Tya Padolih",
        quest = "",
        drop = {
            { zone = "ジャグナー森林", mob = "Za'Dha's Biographer" },
            { zone = "内ホルトト遺跡", mob = "Zurko-Bazurko" },
            { zone = "メリファト山地", mob = "Yowie" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF スカイオーブ 天鳴地響\n印章BF スカイオーブ 剣闘士の得物",
        acquisition = "",
    },
    [109] = {
        name = "リフレシュ",
        description = "標的のパーティメンバーのMPを徐々に回復する。",
        shop = "",
        quest = "",
        drop = {
            { zone = "ラングモント峠", mob = "Ziz" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF スターオーブ 上様御一行\n印章BF スターオーブ 居候妖精\n印章BF スターオーブ 地獄の尻尾\n印章BF スターオーブ 光る眼\n印章BF スターオーブ 人形工房\n印章BF スターオーブ 生ける屍\n印章BF スターオーブ 噂のスライム\n印章BF スターオーブ 地竜大王",
        acquisition = "",
    },
    [110] = {
        name = "リジェネII",
        description = "標的のパーティメンバーのHPを徐々に回復する。",
        shop = "マウラ G-9T1 Tya Padolih",
        quest = "",
        drop = {
            { zone = "外ホルトト遺跡", mob = "Zagh" },
            { zone = "海蛇の岩窟", mob = "Zombie" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [111] = {
        name = "リジェネIII",
        description = "標的のパーティメンバーのHPを徐々に回復する。",
        shop = "",
        quest = "",
        drop = {
            { zone = "クゥルンの大伽藍", mob = "Ziz" },
            { zone = "オンゾゾの迷宮", mob = "Zeid" },
            { zone = "オズトロヤ城", mob = "Yagudo Sentinel" },
            { zone = "ソ・ジヤ", mob = "Yagudo Sentinel" },
            { zone = "テリガン岬", mob = "Zombie" },
            { zone = "クフタルの洞門", mob = "Zeid" },
            { zone = "グスタフの洞門", mob = "Yagudo's Elemental" },
            { zone = "海蛇の岩窟", mob = "Yilbegan" },
            { zone = "海蛇の岩窟", mob = "Zvahl Fortalice" },
            { zone = "イフリートの釜", mob = "Zombie" },
            { zone = "イフリートの釜", mob = "Zoredonite" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF コメットオーブ 爆走爬竜\n印章BF コメットオーブ 三つの葛籠\n印章BF コメットオーブ 虎視眈眈\n印章BF コメットオーブ 戦う羊\n印章BF コメットオーブ 餓鬼武者\n印章BF コメットオーブ 四界の神像\n印章BF コメットオーブ 刻一刻\n印章BF コメットオーブ 拳闘士の獲物",
        acquisition = "",
    },
    [112] = {
        name = "フラッシュ",
        description = "敵の命中率を短時間著しく下げる。",
        shop = "アルザビ J-8T5 Zafif\nラバオ F-7T8 Brave Ox",
        quest = "",
        drop = {
            { zone = "ダボイ", mob = "Yearner" },
            { zone = "ダボイ", mob = "Yowie" },
            { zone = "修道窟", mob = "Zeid" },
            { zone = "修道窟", mob = "Zu" },
            { zone = "ベドー", mob = "Zhayolm Apkallu" },
            { zone = "ズヴァール城外郭", mob = "Zizzy Zillah" },
            { zone = "ズヴァール城外郭", mob = "Zonpa-Zippa" },
            { zone = "ズヴァール城内郭", mob = "Zhayolm Apkallu" },
            { zone = "ズヴァール城内郭", mob = "Ziryu" },
            { zone = "ズヴァール城内郭", mob = "Zurko-Bazurko" },
            { zone = "ズヴァール城内郭", mob = "Zymotic Zuulie" },
            { zone = "東アルテパ砂漠", mob = "Yilbegan" },
            { zone = "東アルテパ砂漠", mob = "Zymotic Zuulie" },
            { zone = "西アルテパ砂漠", mob = "Zizzy Zillah" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [98] = {
        name = "リポーズ",
        description = "敵を眠らせる。",
        shop = "バストゥーク商業区 H-5T1 Sororo",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [312] = {
        name = "エンファイアII",
        description = "初段のみ火の付加ダメージを与え、敵の水属性に対する耐性をダウン。",
        shop = "ル・ルデの庭 H-9T1 Macchi Gazlitah",
        quest = "",
        drop = {
            { zone = "フェ・イン", mob = "Zoraal Ja's Pkuucha" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [313] = {
        name = "エンブリザドII",
        description = "初段のみ氷の付加ダメージを与え、敵の火属性に対する耐性をダウン。",
        shop = "ル・ルデの庭 H-9T1 Macchi Gazlitah",
        quest = "",
        drop = {
            { zone = "北グスタベルグ〔Ｓ〕", mob = "Zolku-Azolku" },
            { zone = "グロウベルグ〔Ｓ〕", mob = "Ze'Vho Fallsplitter" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [314] = {
        name = "エンエアロII",
        description = "初段のみ風の付加ダメージを与え、敵の氷属性に対する耐性をダウン。",
        shop = "ル・ルデの庭 H-9T1 Macchi Gazlitah",
        quest = "",
        drop = {
            { zone = "ガルレージュ要塞", mob = "Zombie" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [315] = {
        name = "エンストーンII",
        description = "初段のみ土の付加ダメージを与え、敵の風属性に対する耐性をダウン。",
        shop = "ル・ルデの庭 H-9T1 Macchi Gazlitah",
        quest = "",
        drop = {
            { zone = "ロ・メーヴ", mob = "Ziz" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [316] = {
        name = "エンサンダーII",
        description = "初段のみ雷の付加ダメージを与え、敵の土属性に対する耐性をダウン。",
        shop = "ル・ルデの庭 H-9T1 Macchi Gazlitah",
        quest = "",
        drop = {
            { zone = "ウガレピ寺院", mob = "Yowie" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [317] = {
        name = "エンウォータII",
        description = "初段のみ水の付加ダメージを与え、敵の雷属性に対する耐性をダウン。",
        shop = "ル・ルデの庭 H-9T1 Macchi Gazlitah",
        quest = "",
        drop = {
            { zone = "ロランベリー耕地〔Ｓ〕", mob = "Yumcax's Maw" },
            { zone = "クロウラーの巣〔Ｓ〕", mob = "Zoredonite" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [120] = {
        name = "テレポヨト",
        description = "範囲内の資格があるパーティメンバーをヨトへワープさせる。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [121] = {
        name = "テレポルテ",
        description = "範囲内の資格があるパーティメンバーをルテへワープさせる。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "辺境 古代魔法の石版",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [122] = {
        name = "テレポホラ",
        description = "範囲内の資格があるパーティメンバーをホラへワープさせる。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [123] = {
        name = "テレポデム",
        description = "範囲内の資格があるパーティメンバーをデムへワープさせる。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "バストゥーク アルタナの悲しみ",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [124] = {
        name = "テレポメア",
        description = "範囲内の資格があるパーティメンバーをメアへワープさせる。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "ウィンダス 信仰の行く先",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [125] = {
        name = "プロテア",
        description = "範囲内にいるパーティメンバーの防御力をアップする。",
        shop = "",
        quest = "",
        drop = {
            { zone = "龍王ランペールの墓", mob = "Zurko-Bazurko" },
            { zone = "バルクルム砂丘", mob = "Young Quadav" },
            { zone = "コンシュタット高地", mob = "Zoredonite" },
            { zone = "オルデール鍾乳洞", mob = "Zonpa-Zippa" },
            { zone = "ジャグナー森林", mob = "Zo'Dhu Legslicer" },
            { zone = "バタリア丘陵", mob = "Zombie" },
            { zone = "北グスタベルグ", mob = "Zurko-Bazurko" },
            { zone = "北グスタベルグ", mob = "Zuu Xowu the Darksmoke" },
            { zone = "南グスタベルグ", mob = "Zurko-Bazurko" },
            { zone = "パルブロ鉱山", mob = "Ziz" },
            { zone = "パシュハウ沼", mob = "Yowie" },
            { zone = "ロランベリー耕地", mob = "Yearner" },
            { zone = "西サルタバルタ", mob = "Zombie" },
            { zone = "東サルタバルタ", mob = "Zombie" },
            { zone = "ギデアス", mob = "Zu" },
            { zone = "内ホルトト遺跡", mob = "Young Quadav" },
            { zone = "ブブリム半島", mob = "Zymotic Zuulie" },
            { zone = "タロンギ大峡谷", mob = "Ze'Vho Fallsplitter" },
            { zone = "シャクラミの地下迷宮", mob = "Zombie" },
            { zone = "メリファト山地", mob = "Yagudo Scribe" },
            { zone = "ソロムグ原野", mob = "Zurko-Bazurko" },
            { zone = "ラングモント峠", mob = "Ziz" },
            { zone = "デルクフの塔下層", mob = "Zoredonite" },
            { zone = "聖地ジ・タ", mob = "Yhoator Wasp" },
            { zone = "北グスタベルグ〔Ｓ〕", mob = "Zizzy Zillah" },
            { zone = "グロウベルグ〔Ｓ〕", mob = "Za'Dha Adamantking" },
            { zone = "グロウベルグ〔Ｓ〕", mob = "Zeid" },
            { zone = "ロランベリー耕地〔Ｓ〕", mob = "Zo'Bha Helmkeeper" },
            { zone = "クロウラーの巣〔Ｓ〕", mob = "Ziz" },
            { zone = "西サルタバルタ〔Ｓ〕", mob = "Zeid" },
            { zone = "カルゴナルゴ城砦〔Ｓ〕", mob = "Zombie" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [126] = {
        name = "プロテアII",
        description = "範囲内にいるパーティメンバーの防御力をアップする。",
        shop = "ジュノ下層 H-9T2 Creepstix\nジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [127] = {
        name = "プロテアIII",
        description = "範囲内にいるパーティメンバーの防御力をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [128] = {
        name = "プロテアIV",
        description = "範囲内にいるパーティメンバーの防御力をアップする。",
        shop = "ウィンダス水の区 南 G-7T3 Shohrun-Tuhrun\nアルザビ J-8T5 Zafif\nラバオ F-7T8 Brave Ox",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "戦績BF プークの幻影\n戦績BF 詠唱中は別の顔\n戦績BF オロボンの呪縛",
        acquisition = "",
    },
    [129] = {
        name = "プロテアV",
        description = "範囲内にいるパーティメンバーの防御力をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [130] = {
        name = "シェルラ",
        description = "範囲内にいるパーティメンバーの魔法防御力をアップする。",
        shop = "ジュノ下層 H-9T2 Creepstix\nジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [131] = {
        name = "シェルラII",
        description = "範囲内にいるパーティメンバーの魔法防御力をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [132] = {
        name = "シェルラIII",
        description = "範囲内にいるパーティメンバーの魔法防御力をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [133] = {
        name = "シェルラIV",
        description = "範囲内にいるパーティメンバーの魔法防御力をアップする。",
        shop = "",
        quest = "",
        drop = {
            { zone = "クゥルンの大伽藍", mob = "Ziz" },
            { zone = "オンゾゾの迷宮", mob = "Zeid" },
            { zone = "オズトロヤ城", mob = "Yagudo Sentinel" },
            { zone = "ソ・ジヤ", mob = "Yagudo Sentinel" },
            { zone = "テリガン岬", mob = "Zombie" },
            { zone = "クフタルの洞門", mob = "Zeid" },
            { zone = "グスタフの洞門", mob = "Yagudo's Elemental" },
            { zone = "海蛇の岩窟", mob = "Yilbegan" },
            { zone = "海蛇の岩窟", mob = "Zvahl Fortalice" },
            { zone = "イフリートの釜", mob = "Zombie" },
            { zone = "イフリートの釜", mob = "Zoredonite" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [134] = {
        name = "シェルラV",
        description = "範囲内にいるパーティメンバーの魔法防御力をアップする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [135] = {
        name = "リレイズ",
        description = "戦闘不能時にレイズの効果が発動する。",
        shop = "",
        quest = "",
        drop = {
            { zone = "ベドー", mob = "Zolku-Azolku" },
            { zone = "ベドー", mob = "Zu" },
            { zone = "オズトロヤ城", mob = "Yearner" },
            { zone = "ズヴァール城内郭", mob = "Zolku-Azolku" },
            { zone = "ズヴァール城内郭", mob = "Zombie" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [136] = {
        name = "インビジ",
        description = "透明になり敵に発見されにくくする。",
        shop = "セルビナ H-9T5 Falgima\nマウラ G-9T1 Tya Padolih",
        quest = "",
        drop = {
            { zone = "龍王ランペールの墓", mob = "Zurko-Bazurko" },
            { zone = "バルクルム砂丘", mob = "Young Quadav" },
            { zone = "オルデール鍾乳洞", mob = "Zonpa-Zippa" },
            { zone = "ジャグナー森林", mob = "Zo'Dhu Legslicer" },
            { zone = "バタリア丘陵", mob = "Zombie" },
            { zone = "北グスタベルグ", mob = "Zuu Xowu the Darksmoke" },
            { zone = "パシュハウ沼", mob = "Yowie" },
            { zone = "ロランベリー耕地", mob = "Yearner" },
            { zone = "ロランベリー耕地", mob = "Zombie" },
            { zone = "ベドー", mob = "Yagudo Scribe" },
            { zone = "ギデアス", mob = "Yearner" },
            { zone = "内ホルトト遺跡", mob = "Young Quadav" },
            { zone = "ブブリム半島", mob = "Zymotic Zuulie" },
            { zone = "シャクラミの地下迷宮", mob = "Zombie" },
            { zone = "メリファト山地", mob = "Yagudo Scribe" },
            { zone = "メリファト山地", mob = "Zeid" },
            { zone = "ソロムグ原野", mob = "Zonpa-Zippa" },
            { zone = "ソロムグ原野", mob = "Zurko-Bazurko" },
            { zone = "オズトロヤ城", mob = "Zeid" },
            { zone = "ラングモント峠", mob = "Ziz" },
            { zone = "デルクフの塔下層", mob = "Zoredonite" },
            { zone = "聖地ジ・タ", mob = "Yhoator Wasp" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF クラウディオーブ 闇夜の舞姫",
        acquisition = "",
    },
    [137] = {
        name = "スニーク",
        description = "音を消し敵に発見されにくくする。",
        shop = "セルビナ H-9T5 Falgima\nマウラ G-9T1 Tya Padolih",
        quest = "",
        drop = {
            { zone = "龍王ランペールの墓", mob = "Zurko-Bazurko" },
            { zone = "バルクルム砂丘", mob = "Young Quadav" },
            { zone = "コンシュタット高地", mob = "Zoredonite" },
            { zone = "オルデール鍾乳洞", mob = "Zonpa-Zippa" },
            { zone = "ジャグナー森林", mob = "Zo'Dhu Legslicer" },
            { zone = "バタリア丘陵", mob = "Zombie" },
            { zone = "北グスタベルグ", mob = "Zurko-Bazurko" },
            { zone = "北グスタベルグ", mob = "Zuu Xowu the Darksmoke" },
            { zone = "南グスタベルグ", mob = "Zurko-Bazurko" },
            { zone = "パルブロ鉱山", mob = "Ziz" },
            { zone = "パシュハウ沼", mob = "Yowie" },
            { zone = "ロランベリー耕地", mob = "Yearner" },
            { zone = "ベドー", mob = "Yagudo Scribe" },
            { zone = "西サルタバルタ", mob = "Zombie" },
            { zone = "東サルタバルタ", mob = "Zombie" },
            { zone = "ギデアス", mob = "Zu" },
            { zone = "内ホルトト遺跡", mob = "Young Quadav" },
            { zone = "ブブリム半島", mob = "Zymotic Zuulie" },
            { zone = "タロンギ大峡谷", mob = "Ze'Vho Fallsplitter" },
            { zone = "シャクラミの地下迷宮", mob = "Zombie" },
            { zone = "メリファト山地", mob = "Yagudo Scribe" },
            { zone = "ソロムグ原野", mob = "Zurko-Bazurko" },
            { zone = "ラングモント峠", mob = "Ziz" },
            { zone = "デルクフの塔下層", mob = "Zoredonite" },
            { zone = "聖地ジ・タ", mob = "Yhoator Wasp" },
            { zone = "北グスタベルグ〔Ｓ〕", mob = "Zizzy Zillah" },
            { zone = "グロウベルグ〔Ｓ〕", mob = "Za'Dha Adamantking" },
            { zone = "グロウベルグ〔Ｓ〕", mob = "Zeid" },
            { zone = "ロランベリー耕地〔Ｓ〕", mob = "Zo'Bha Helmkeeper" },
            { zone = "クロウラーの巣〔Ｓ〕", mob = "Ziz" },
            { zone = "西サルタバルタ〔Ｓ〕", mob = "Zeid" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF クラウディオーブ 闇夜の舞姫",
        acquisition = "",
    },
    [138] = {
        name = "デオード",
        description = "臭いを消しモンスターに追跡されにくくする。",
        shop = "セルビナ H-9T5 Falgima\nマウラ G-9T1 Tya Padolih",
        quest = "",
        drop = {
            { zone = "龍王ランペールの墓", mob = "Zurko-Bazurko" },
            { zone = "バルクルム砂丘", mob = "Young Quadav" },
            { zone = "オルデール鍾乳洞", mob = "Zonpa-Zippa" },
            { zone = "ジャグナー森林", mob = "Zo'Dhu Legslicer" },
            { zone = "バタリア丘陵", mob = "Zombie" },
            { zone = "北グスタベルグ", mob = "Zuu Xowu the Darksmoke" },
            { zone = "パシュハウ沼", mob = "Yowie" },
            { zone = "ロランベリー耕地", mob = "Yearner" },
            { zone = "ロランベリー耕地", mob = "Zombie" },
            { zone = "ギデアス", mob = "Yearner" },
            { zone = "ギデアス", mob = "Ze'Vho Fallsplitter" },
            { zone = "内ホルトト遺跡", mob = "Young Quadav" },
            { zone = "ブブリム半島", mob = "Zymotic Zuulie" },
            { zone = "シャクラミの地下迷宮", mob = "Zombie" },
            { zone = "メリファト山地", mob = "Yagudo Scribe" },
            { zone = "メリファト山地", mob = "Zeid" },
            { zone = "ソロムグ原野", mob = "Zonpa-Zippa" },
            { zone = "ソロムグ原野", mob = "Zurko-Bazurko" },
            { zone = "オズトロヤ城", mob = "Zeid" },
            { zone = "ラングモント峠", mob = "Ziz" },
            { zone = "デルクフの塔下層", mob = "Zoredonite" },
            { zone = "聖地ジ・タ", mob = "Yhoator Wasp" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF クラウディオーブ 闇夜の舞姫",
        acquisition = "",
    },
    [139] = {
        name = "テレポヴァズ",
        description = "範囲内の資格があるパーティメンバーをヴァズへワープさせる。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [140] = {
        name = "レイズIII",
        description = "標的の戦闘不能状態を回復。",
        shop = "",
        quest = "",
        drop = {
            { zone = "クゥルンの大伽藍", mob = "Zu" },
            { zone = "ル・アビタウ神殿", mob = "Zhayolm Apkallu" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "ENM 唇亡びて\nENM ウルガランの聖牛\nENM 雪こんこん\n印章BF クローソーオーブ 伝説のチャンピオン\n印章BF ラキシスオーブ 飛竜の眷族\n印章BF アトロポスオーブ 紅蓮の翼\nENM 幻の右\n印章BF クローソーオーブ 蜘蛛の神\n印章BF ラキシスオーブ 破滅の壺\n印章BF アトロポスオーブ 蝿の王",
        acquisition = "",
    },
    [141] = {
        name = "リレイズII",
        description = "戦闘不能時にレイズIIの効果が発動する。",
        shop = "",
        quest = "",
        drop = {
            { zone = "フェ・イン", mob = "Yowie" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF ムーンオーブ ドーフェ兄弟\n印章BF ムーンオーブ 攻城剛力組\n印章BF ムーンオーブ 蒼の血族",
        acquisition = "",
    },
    [142] = {
        name = "リレイズIII",
        description = "戦闘不能時にレイズIIIの効果が発動する。",
        shop = "アルザビ J-8T5 Zafif\nラバオ F-7T8 Brave Ox",
        quest = "",
        drop = {
            { zone = "オンゾゾの迷宮", mob = "Zeid" },
            { zone = "オズトロヤ城", mob = "Yagudo Sentinel" },
            { zone = "ソ・ジヤ", mob = "Yagudo Sentinel" },
            { zone = "テリガン岬", mob = "Zombie" },
            { zone = "クフタルの洞門", mob = "Zeid" },
            { zone = "グスタフの洞門", mob = "Yagudo's Elemental" },
            { zone = "海蛇の岩窟", mob = "Yilbegan" },
            { zone = "海蛇の岩窟", mob = "Zolku-Azolku" },
            { zone = "イフリートの釜", mob = "Zoredonite" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "戦績BF プークの幻影\n戦績BF 詠唱中は別の顔\n戦績BF オロボンの呪縛",
        acquisition = "",
    },
    [143] = {
        name = "イレース",
        description = "標的のパーティメンバーにかかっている悪い魔法効果を1つ取り除く。",
        shop = "",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF スカイオーブ 断食芋虫\n印章BF スカイオーブ 天鳴地響\n印章BF スターオーブ 上様御一行\n印章BF スターオーブ 居候妖精\n印章BF ムーンオーブ 第11軍団独立支隊\n印章BF スカイオーブ 石竜の双璧\n印章BF スカイオーブ 山岳甲兵\n印章BF スカイオーブ 降下猟兵\n印章BF スターオーブ 地獄の尻尾\n印章BF スターオーブ 光る眼\n戦績BF プークの幻影\n戦績BF 詠唱中は別の顔\n印章BF スカイオーブ 剣闘士の得物\n印章BF スターオーブ 人形工房\n印章BF スカイオーブ 凶狗四天王\n印章BF スターオーブ 生ける屍\n印章BF ムーンオーブ 潜行特務隊\n印章BF ムーンオーブ 特命介錯人\n戦績BF オロボンの呪縛\n印章BF スカイオーブ 飛行小隊\n印章BF スカイオーブ 森の乙女\n印章BF スターオーブ 噂のスライム\n印章BF スターオーブ 地竜大王",
        acquisition = "",
    },
    [144] = {
        name = "ファイア",
        description = "敵に火属性のダメージ。",
        shop = "バストゥーク商業区 H-5T1 Zaira\nウィンダス港 H-8T8 Aroro",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [145] = {
        name = "ファイアII",
        description = "敵に火属性のダメージ。",
        shop = "ウィンダス水の区 南 G-7T3 Hilkomu-Makimu\nセルビナ H-9T5 Chutarmire",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF スカイオーブ 山岳甲兵",
        acquisition = "",
    },
    [146] = {
        name = "ファイアIII",
        description = "敵に火属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu\nアトルガン白門 G-7T7 Mazween",
        quest = "",
        drop = {
            { zone = "龍王ランペールの墓", mob = "Zirnitra" },
            { zone = "ズヴァール城外郭", mob = "Yumcax's Maw" },
            { zone = "ズヴァール城外郭", mob = "Zjaa Bao the Wrathherald" },
            { zone = "ウルガラン山脈", mob = "Zoraal Ja's Pkuucha" },
            { zone = "海蛇の岩窟", mob = "Zuu Feya the Impassive" },
            { zone = "怨念洞", mob = "Zhu Que" },
            { zone = "怨念洞", mob = "Zuu Xowu the Darksmoke" },
            { zone = "イフリートの釜", mob = "Yhoator Wasp" },
            { zone = "ザルカバード〔Ｓ〕", mob = "Zoraal Ja's Pkuucha" },
            { zone = "ズヴァール城外郭〔Ｓ〕", mob = "Za'Dha Adamantking" },
            { zone = "ズヴァール城内郭〔Ｓ〕", mob = "Zo'Bha Helmkeeper" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF ムーンオーブ 蒼の血族",
        acquisition = "",
    },
    [147] = {
        name = "ファイアIV",
        description = "敵に火属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "ズヴァール城外郭", mob = "Yumcax's Maw" },
            { zone = "ズヴァール城外郭", mob = "Zjaa Bao the Wrathherald" },
            { zone = "ウルガラン山脈", mob = "Zoraal Ja's Pkuucha" },
            { zone = "怨念洞", mob = "Zhu Que" },
            { zone = "怨念洞", mob = "Zuu Xowu the Darksmoke" },
            { zone = "ザルカバード〔Ｓ〕", mob = "Zoraal Ja's Pkuucha" },
            { zone = "ズヴァール城外郭〔Ｓ〕", mob = "Za'Dha Adamantking" },
            { zone = "ズヴァール城内郭〔Ｓ〕", mob = "Zo'Bha Helmkeeper" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [148] = {
        name = "ファイアV",
        description = "敵に火属性のダメージ。",
        shop = "ウィンダス水の区〔Ｓ〕 南 H-8T7 Ezura-Romazura",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [149] = {
        name = "ブリザド",
        description = "敵に氷属性のダメージ。",
        shop = "バストゥーク商業区 H-5T1 Zaira\nウィンダス港 H-8T8 Aroro",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [150] = {
        name = "ブリザドII",
        description = "敵に氷属性のダメージ。",
        shop = "ウィンダス水の区 南 G-7T3 Hilkomu-Makimu\nセルビナ H-9T5 Chutarmire",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [151] = {
        name = "ブリザドIII",
        description = "敵に氷属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu\nアトルガン白門 G-7T7 Mazween",
        quest = "",
        drop = {
            { zone = "龍王ランペールの墓", mob = "Yowie" },
            { zone = "龍王ランペールの墓", mob = "Zuug the Shoreleaper" },
            { zone = "エルディーム古墳", mob = "Zvahl Fortalice" },
            { zone = "トライマライ水路", mob = "Zombie" },
            { zone = "ガルレージュ要塞", mob = "Yara Ma Yha Who" },
            { zone = "アットワ地溝", mob = "Zhu Que" },
            { zone = "ウルガラン山脈", mob = "Zolku-Azolku" },
            { zone = "テリガン岬", mob = "Yagudo Scribe" },
            { zone = "グスタフの洞門", mob = "Young Quadav" },
            { zone = "グスタフの洞門", mob = "Zurko-Bazurko" },
            { zone = "グスタフの洞門", mob = "Zuu Xowu the Darksmoke" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [152] = {
        name = "ブリザドIV",
        description = "敵に氷属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "龍王ランペールの墓", mob = "Yowie" },
            { zone = "龍王ランペールの墓", mob = "Zuug the Shoreleaper" },
            { zone = "グスタフの洞門", mob = "Zurko-Bazurko" },
            { zone = "グスタフの洞門", mob = "Zuu Xowu the Darksmoke" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "戦績BF プークの幻影\n戦績BF 詠唱中は別の顔\n戦績BF オロボンの呪縛",
        acquisition = "",
    },
    [153] = {
        name = "ブリザドV",
        description = "敵に氷属性のダメージ。",
        shop = "ウィンダス水の区〔Ｓ〕 南 H-8T7 Ezura-Romazura",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [154] = {
        name = "エアロ",
        description = "敵に風属性のダメージ。",
        shop = "バストゥーク商業区 H-5T1 Zaira\nウィンダス港 H-8T8 Aroro",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [155] = {
        name = "エアロII",
        description = "敵に風属性のダメージ。",
        shop = "ウィンダス水の区 南 G-7T3 Hilkomu-Makimu\nセルビナ H-9T5 Chutarmire",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [156] = {
        name = "エアロIII",
        description = "敵に風属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu\nアトルガン白門 G-7T7 Mazween",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Yilbegan" },
            { zone = "オズトロヤ城", mob = "Yowie" },
            { zone = "ムバルポロス新市街", mob = "Yearner" },
            { zone = "ムバルポロス新市街", mob = "Zombie" },
            { zone = "メリファト山地〔Ｓ〕", mob = "Ziz" },
            { zone = "ソロムグ原野〔Ｓ〕", mob = "Zurko-Bazurko" },
            { zone = "オズトロヤ城〔Ｓ〕", mob = "Zeid" },
            { zone = "オズトロヤ城〔Ｓ〕", mob = "Ziz" },
            { zone = "ガルレージュ要塞〔Ｓ〕", mob = "Zoldeff Jagil" },
            { zone = "ズヴァール城外郭〔Ｓ〕", mob = "Yearner" },
            { zone = "ズヴァール城内郭〔Ｓ〕", mob = "Zizzy Zillah" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [157] = {
        name = "エアロIV",
        description = "敵に風属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Yilbegan" },
            { zone = "オズトロヤ城", mob = "Yowie" },
            { zone = "ムバルポロス新市街", mob = "Yearner" },
            { zone = "ムバルポロス新市街", mob = "Zombie" },
            { zone = "メリファト山地〔Ｓ〕", mob = "Ziz" },
            { zone = "ソロムグ原野〔Ｓ〕", mob = "Zurko-Bazurko" },
            { zone = "オズトロヤ城〔Ｓ〕", mob = "Zeid" },
            { zone = "オズトロヤ城〔Ｓ〕", mob = "Ziz" },
            { zone = "ガルレージュ要塞〔Ｓ〕", mob = "Zoldeff Jagil" },
            { zone = "ズヴァール城外郭〔Ｓ〕", mob = "Yearner" },
            { zone = "ズヴァール城内郭〔Ｓ〕", mob = "Zizzy Zillah" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "戦績BF ひとまねことり\n戦績BF ローリングバグズ\n戦績BF タラッカの笛吹き",
        acquisition = "",
    },
    [158] = {
        name = "エアロV",
        description = "敵に風属性のダメージ。",
        shop = "ウィンダス水の区〔Ｓ〕 南 H-8T7 Ezura-Romazura",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [159] = {
        name = "ストーン",
        description = "敵に土属性のダメージ。",
        shop = "バストゥーク商業区 H-5T1 Zaira\nウィンダス港 H-8T8 Aroro",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "黒魔道士の初期装備",
    },
    [160] = {
        name = "ストーンII",
        description = "敵に土属性のダメージ。",
        shop = "ウィンダス水の区 南 G-7T3 Hilkomu-Makimu\nセルビナ H-9T5 Chutarmire",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [161] = {
        name = "ストーンIII",
        description = "敵に土属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu\nアトルガン白門 G-7T7 Mazween",
        quest = "",
        drop = {
            { zone = "修道窟", mob = "Yagudo Scribe" },
            { zone = "修道窟", mob = "Ziz" },
            { zone = "流砂洞", mob = "Yagudo Scribe" },
            { zone = "ムバルポロス新市街", mob = "Yhoator Wasp" },
            { zone = "北グスタベルグ〔Ｓ〕", mob = "Zuu Xowu the Darksmoke" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [162] = {
        name = "ストーンIV",
        description = "敵に土属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "修道窟", mob = "Yagudo Scribe" },
            { zone = "修道窟", mob = "Ziz" },
            { zone = "流砂洞", mob = "Yagudo Scribe" },
            { zone = "流砂洞", mob = "Young Quadav" },
            { zone = "流砂洞", mob = "Yowie" },
            { zone = "流砂洞", mob = "Ze'Vho Fallsplitter" },
            { zone = "北グスタベルグ〔Ｓ〕", mob = "Zuu Xowu the Darksmoke" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [163] = {
        name = "ストーンV",
        description = "敵に土属性のダメージ。",
        shop = "ウィンダス水の区〔Ｓ〕 南 H-8T7 Ezura-Romazura",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [164] = {
        name = "サンダー",
        description = "敵に雷属性のダメージ。",
        shop = "バストゥーク商業区 H-5T1 Zaira\nウィンダス港 H-8T8 Aroro",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [165] = {
        name = "サンダーII",
        description = "敵に雷属性のダメージ。",
        shop = "ウィンダス水の区 南 G-7T3 Hilkomu-Makimu\nセルビナ H-9T5 Chutarmire",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [166] = {
        name = "サンダーIII",
        description = "敵に雷属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu\nアトルガン白門 G-7T7 Mazween",
        quest = "",
        drop = {
            { zone = "オンゾゾの迷宮", mob = "Yagudo Scribe" },
            { zone = "デルクフの塔上層", mob = "Ziz" },
            { zone = "ロ・メーヴ", mob = "Ziz" },
            { zone = "ヨアトル大森林", mob = "Yara Ma Yha Who" },
            { zone = "ウガレピ寺院", mob = "Yowie" },
            { zone = "ウガレピ寺院", mob = "Zhayolm Apkallu" },
            { zone = "怨念洞", mob = "Yhoator Wasp" },
            { zone = "ル・オンの庭", mob = "Yagudo Votary" },
            { zone = "ヴェ・ルガノン宮殿", mob = "Ziz" },
            { zone = "ル・アビタウ神殿", mob = "Zazalda Clot" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF クローソーオーブ 伝説のチャンピオン\n印章BF ラキシスオーブ 飛竜の眷族\n印章BF アトロポスオーブ 紅蓮の翼\n印章BF クローソーオーブ 蜘蛛の神\n印章BF ラキシスオーブ 破滅の壺\n印章BF アトロポスオーブ 蝿の王",
        acquisition = "",
    },
    [167] = {
        name = "サンダーIV",
        description = "敵に雷属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "オンゾゾの迷宮", mob = "Yagudo Scribe" },
            { zone = "デルクフの塔上層", mob = "Ziz" },
            { zone = "ロ・メーヴ", mob = "Ziz" },
            { zone = "ウガレピ寺院", mob = "Yowie" },
            { zone = "ル・オンの庭", mob = "Yagudo Votary" },
            { zone = "ヴェ・ルガノン宮殿", mob = "Ziz" },
            { zone = "ル・アビタウ神殿", mob = "Zazalda Clot" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [169] = {
        name = "ウォータ",
        description = "敵に水属性のダメージ。",
        shop = "バストゥーク商業区 H-5T1 Zaira\nウィンダス港 H-8T8 Aroro",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [170] = {
        name = "ウォータII",
        description = "敵に水属性のダメージ。",
        shop = "ウィンダス水の区 南 G-7T3 Hilkomu-Makimu\nセルビナ H-9T5 Chutarmire",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [171] = {
        name = "ウォータIII",
        description = "敵に水属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu\nアトルガン白門 G-7T7 Mazween",
        quest = "",
        drop = {
            { zone = "ベドー", mob = "Yagudo Sentinel" },
            { zone = "クゥルンの大伽藍", mob = "Zeid" },
            { zone = "怨念洞", mob = "Yagudo Sentinel" },
            { zone = "怨念洞", mob = "Yearner" },
            { zone = "ベドー〔Ｓ〕", mob = "Young Quadav" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [172] = {
        name = "ウォータIV",
        description = "敵に水属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "ベドー", mob = "Yagudo Sentinel" },
            { zone = "クゥルンの大伽藍", mob = "Zeid" },
            { zone = "怨念洞", mob = "Yagudo Sentinel" },
            { zone = "怨念洞", mob = "Yearner" },
            { zone = "怨念洞", mob = "Zoredonite" },
            { zone = "ベドー〔Ｓ〕", mob = "Young Quadav" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "戦績BF プークの幻影\n戦績BF 詠唱中は別の顔\n戦績BF オロボンの呪縛",
        acquisition = "",
    },
    [173] = {
        name = "ウォータV",
        description = "敵に水属性のダメージ。",
        shop = "ウィンダス水の区〔Ｓ〕 南 H-8T7 Ezura-Romazura",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [174] = {
        name = "ファイガ",
        description = "範囲内の敵に火属性のダメージ。",
        shop = "ウィンダス水の区 南 G-7T3 Hilkomu-Makimu\nセルビナ H-9T5 Chutarmire",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [175] = {
        name = "ファイガII",
        description = "範囲内の敵に火属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "ズヴァール城外郭", mob = "Yumcax's Maw" },
            { zone = "ウルガラン山脈", mob = "Yara Ma Yha Who" },
            { zone = "ウルガラン山脈", mob = "Zoraal Ja's Pkuucha" },
            { zone = "ザルカバード〔Ｓ〕", mob = "Zoraal Ja's Pkuucha" },
            { zone = "ズヴァール城外郭〔Ｓ〕", mob = "Za'Dha Adamantking" },
            { zone = "ズヴァール城内郭〔Ｓ〕", mob = "Zo'Bha Helmkeeper" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [176] = {
        name = "ファイガIII",
        description = "範囲内の敵に火属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "ズヴァール城外郭", mob = "Yumcax's Maw" },
            { zone = "ズヴァール城外郭", mob = "Zjaa Bao the Wrathherald" },
            { zone = "ウルガラン山脈", mob = "Zoraal Ja's Pkuucha" },
            { zone = "怨念洞", mob = "Zhu Que" },
            { zone = "怨念洞", mob = "Zuu Xowu the Darksmoke" },
            { zone = "ザルカバード〔Ｓ〕", mob = "Zoraal Ja's Pkuucha" },
            { zone = "ズヴァール城外郭〔Ｓ〕", mob = "Za'Dha Adamantking" },
            { zone = "ズヴァール城内郭〔Ｓ〕", mob = "Zo'Bha Helmkeeper" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [179] = {
        name = "ブリザガ",
        description = "範囲内の敵に氷属性のダメージ。",
        shop = "ウィンダス水の区 南 G-7T3 Hilkomu-Makimu\nセルビナ H-9T5 Chutarmire",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [180] = {
        name = "ブリザガII",
        description = "範囲内の敵に氷属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "龍王ランペールの墓", mob = "Yowie" },
            { zone = "龍王ランペールの墓", mob = "Zuug the Shoreleaper" },
            { zone = "エルディーム古墳", mob = "Zo'Bha Helmkeeper" },
            { zone = "ウルガラン山脈", mob = "Zo'Dhu Legslicer" },
            { zone = "ウルガラン山脈", mob = "Zombie" },
            { zone = "グスタフの洞門", mob = "Zurko-Bazurko" },
            { zone = "グスタフの洞門", mob = "Zuu Xowu the Darksmoke" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [181] = {
        name = "ブリザガIII",
        description = "範囲内の敵に氷属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "龍王ランペールの墓", mob = "Yowie" },
            { zone = "龍王ランペールの墓", mob = "Zuug the Shoreleaper" },
            { zone = "グスタフの洞門", mob = "Zurko-Bazurko" },
            { zone = "グスタフの洞門", mob = "Zuu Xowu the Darksmoke" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [184] = {
        name = "エアロガ",
        description = "範囲内の敵に風属性のダメージ。",
        shop = "ウィンダス水の区 南 G-7T3 Hilkomu-Makimu\nセルビナ H-9T5 Chutarmire",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [185] = {
        name = "エアロガII",
        description = "範囲内の敵に風属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Yilbegan" },
            { zone = "オズトロヤ城", mob = "Yowie" },
            { zone = "ムバルポロス新市街", mob = "Yearner" },
            { zone = "ムバルポロス新市街", mob = "Zombie" },
            { zone = "メリファト山地〔Ｓ〕", mob = "Ziz" },
            { zone = "ソロムグ原野〔Ｓ〕", mob = "Zurko-Bazurko" },
            { zone = "オズトロヤ城〔Ｓ〕", mob = "Zeid" },
            { zone = "オズトロヤ城〔Ｓ〕", mob = "Ziz" },
            { zone = "ガルレージュ要塞〔Ｓ〕", mob = "Zoldeff Jagil" },
            { zone = "ズヴァール城外郭〔Ｓ〕", mob = "Yearner" },
            { zone = "ズヴァール城内郭〔Ｓ〕", mob = "Zizzy Zillah" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [186] = {
        name = "エアロガIII",
        description = "範囲内の敵に風属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Yilbegan" },
            { zone = "オズトロヤ城", mob = "Yowie" },
            { zone = "ズヴァール城内郭", mob = "Zoldeff Jagil" },
            { zone = "ムバルポロス新市街", mob = "Yearner" },
            { zone = "ムバルポロス新市街", mob = "Zombie" },
            { zone = "メリファト山地〔Ｓ〕", mob = "Ziz" },
            { zone = "ソロムグ原野〔Ｓ〕", mob = "Zurko-Bazurko" },
            { zone = "オズトロヤ城〔Ｓ〕", mob = "Zeid" },
            { zone = "オズトロヤ城〔Ｓ〕", mob = "Ziz" },
            { zone = "ガルレージュ要塞〔Ｓ〕", mob = "Zoldeff Jagil" },
            { zone = "ズヴァール城外郭〔Ｓ〕", mob = "Yearner" },
            { zone = "ズヴァール城内郭〔Ｓ〕", mob = "Zizzy Zillah" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [189] = {
        name = "ストンガ",
        description = "範囲内の敵に土属性のダメージ。",
        shop = "ウィンダス水の区 南 G-7T3 Hilkomu-Makimu\nセルビナ H-9T5 Chutarmire",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [190] = {
        name = "ストンガII",
        description = "範囲内の敵に土属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "修道窟", mob = "Yagudo Scribe" },
            { zone = "修道窟", mob = "Ziz" },
            { zone = "流砂洞", mob = "Yagudo Scribe" },
            { zone = "流砂洞", mob = "Ze'Vho Fallsplitter" },
            { zone = "ムバルポロス新市街", mob = "Yhoator Wasp" },
            { zone = "北グスタベルグ〔Ｓ〕", mob = "Zuu Xowu the Darksmoke" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [191] = {
        name = "ストンガIII",
        description = "範囲内の敵に土属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "修道窟", mob = "Yagudo Scribe" },
            { zone = "修道窟", mob = "Ziz" },
            { zone = "流砂洞", mob = "Yagudo Scribe" },
            { zone = "流砂洞", mob = "Young Quadav" },
            { zone = "流砂洞", mob = "Yowie" },
            { zone = "流砂洞", mob = "Ze'Vho Fallsplitter" },
            { zone = "ムバルポロス新市街", mob = "Yhoator Wasp" },
            { zone = "北グスタベルグ〔Ｓ〕", mob = "Zuu Xowu the Darksmoke" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [194] = {
        name = "サンダガ",
        description = "範囲内の敵に雷属性のダメージ。",
        shop = "ウィンダス水の区 南 G-7T3 Hilkomu-Makimu\nセルビナ H-9T5 Chutarmire",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [195] = {
        name = "サンダガII",
        description = "範囲内の敵に雷属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "ズヴァール城内郭", mob = "Zoldeff Jagil" },
            { zone = "ヨアトル大森林", mob = "Yara Ma Yha Who" },
            { zone = "ウガレピ寺院", mob = "Zhayolm Apkallu" },
            { zone = "怨念洞", mob = "Yhoator Wasp" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [196] = {
        name = "サンダガIII",
        description = "範囲内の敵に雷属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "オンゾゾの迷宮", mob = "Yagudo Scribe" },
            { zone = "デルクフの塔上層", mob = "Ziz" },
            { zone = "ロ・メーヴ", mob = "Ziz" },
            { zone = "ウガレピ寺院", mob = "Yowie" },
            { zone = "ル・オンの庭", mob = "Yagudo Votary" },
            { zone = "ヴェ・ルガノン宮殿", mob = "Ziz" },
            { zone = "ル・アビタウ神殿", mob = "Zazalda Clot" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [199] = {
        name = "ウォタガ",
        description = "範囲内の敵に水属性のダメージ。",
        shop = "ウィンダス水の区 南 G-7T3 Hilkomu-Makimu\nセルビナ H-9T5 Chutarmire",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [200] = {
        name = "ウォタガII",
        description = "範囲内の敵に水属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "ベドー", mob = "Yagudo Sentinel" },
            { zone = "クゥルンの大伽藍", mob = "Zeid" },
            { zone = "怨念洞", mob = "Yagudo Sentinel" },
            { zone = "怨念洞", mob = "Yearner" },
            { zone = "ベドー〔Ｓ〕", mob = "Young Quadav" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [201] = {
        name = "ウォタガIII",
        description = "範囲内の敵に水属性のダメージ。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "ベドー", mob = "Yagudo Sentinel" },
            { zone = "クゥルンの大伽藍", mob = "Zeid" },
            { zone = "怨念洞", mob = "Yagudo Sentinel" },
            { zone = "怨念洞", mob = "Yearner" },
            { zone = "怨念洞", mob = "Zoredonite" },
            { zone = "ベドー〔Ｓ〕", mob = "Young Quadav" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [204] = {
        name = "フレア",
        description = "敵に火属性のダメージを与え、水属性に対する防御力をダウン。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "ズヴァール城外郭", mob = "Yumcax's Maw" },
            { zone = "ズヴァール城外郭", mob = "Zjaa Bao the Wrathherald" },
            { zone = "ウルガラン山脈", mob = "Zoraal Ja's Pkuucha" },
            { zone = "怨念洞", mob = "Zhu Que" },
            { zone = "怨念洞", mob = "Zuu Xowu the Darksmoke" },
            { zone = "ザルカバード〔Ｓ〕", mob = "Zoraal Ja's Pkuucha" },
            { zone = "ズヴァール城外郭〔Ｓ〕", mob = "Za'Dha Adamantking" },
            { zone = "ズヴァール城内郭〔Ｓ〕", mob = "Zo'Bha Helmkeeper" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "戦績BF ひとまねことり\n戦績BF ローリングバグズ\n印章BF ムーンオーブ ドーフェ兄弟\n印章BF ムーンオーブ 攻城剛力組\n戦績BF タラッカの笛吹き",
        acquisition = "",
    },
    [205] = {
        name = "フレアII",
        description = "敵に火属性のダメージを与え、水属性に対する防御力をダウン。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [206] = {
        name = "フリーズ",
        description = "敵に氷属性のダメージを与え、火属性に対する防御力をダウン。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "龍王ランペールの墓", mob = "Yowie" },
            { zone = "龍王ランペールの墓", mob = "Zuug the Shoreleaper" },
            { zone = "エルディーム古墳", mob = "Zo'Bha Helmkeeper" },
            { zone = "グスタフの洞門", mob = "Zurko-Bazurko" },
            { zone = "グスタフの洞門", mob = "Zuu Xowu the Darksmoke" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF コメットオーブ 爆走爬竜\n印章BF コメットオーブ 三つの葛籠\n印章BF コメットオーブ 虎視眈眈\n印章BF コメットオーブ 戦う羊\n印章BF コメットオーブ 餓鬼武者\n印章BF コメットオーブ 四界の神像\n印章BF コメットオーブ 刻一刻\n印章BF コメットオーブ 拳闘士の獲物",
        acquisition = "",
    },
    [207] = {
        name = "フリーズII",
        description = "敵に氷属性のダメージを与え、火属性に対する防御力をダウン。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [208] = {
        name = "トルネド",
        description = "敵に風属性のダメージを与え、氷属性に対する防御力をダウン。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Yilbegan" },
            { zone = "オズトロヤ城", mob = "Yowie" },
            { zone = "メリファト山地〔Ｓ〕", mob = "Ziz" },
            { zone = "ソロムグ原野〔Ｓ〕", mob = "Zurko-Bazurko" },
            { zone = "オズトロヤ城〔Ｓ〕", mob = "Zeid" },
            { zone = "オズトロヤ城〔Ｓ〕", mob = "Ziz" },
            { zone = "ガルレージュ要塞〔Ｓ〕", mob = "Zoldeff Jagil" },
            { zone = "ズヴァール城外郭〔Ｓ〕", mob = "Yearner" },
            { zone = "ズヴァール城内郭〔Ｓ〕", mob = "Zizzy Zillah" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "戦績BF ひとまねことり\n戦績BF ローリングバグズ\n戦績BF タラッカの笛吹き",
        acquisition = "",
    },
    [209] = {
        name = "トルネドII",
        description = "敵に風属性のダメージを与え、氷属性に対する防御力をダウン。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [210] = {
        name = "クエイク",
        description = "敵に土属性のダメージを与え、風属性に対する防御力をダウン。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "修道窟", mob = "Yagudo Scribe" },
            { zone = "修道窟", mob = "Ziz" },
            { zone = "流砂洞", mob = "Yagudo Scribe" },
            { zone = "ル・アビタウ神殿", mob = "Zhayolm Apkallu" },
            { zone = "ムバルポロス新市街", mob = "Yhoator Wasp" },
            { zone = "北グスタベルグ〔Ｓ〕", mob = "Zuu Xowu the Darksmoke" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF コメットオーブ 爆走爬竜\n印章BF コメットオーブ 三つの葛籠\n印章BF コメットオーブ 虎視眈眈\n印章BF コメットオーブ 戦う羊\n戦績BF プークの幻影\n戦績BF 詠唱中は別の顔\n印章BF コメットオーブ 餓鬼武者\n印章BF コメットオーブ 四界の神像\n戦績BF オロボンの呪縛\n印章BF コメットオーブ 刻一刻\n印章BF コメットオーブ 拳闘士の獲物",
        acquisition = "",
    },
    [211] = {
        name = "クエイクII",
        description = "敵に土属性のダメージを与え、風属性に対する防御力をダウン。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [212] = {
        name = "バースト",
        description = "敵に雷属性のダメージを与え、土属性に対する防御力をダウン。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "オンゾゾの迷宮", mob = "Yagudo Scribe" },
            { zone = "デルクフの塔上層", mob = "Ziz" },
            { zone = "ロ・メーヴ", mob = "Ziz" },
            { zone = "ウガレピ寺院", mob = "Yowie" },
            { zone = "ル・オンの庭", mob = "Yagudo Votary" },
            { zone = "ヴェ・ルガノン宮殿", mob = "Ziz" },
            { zone = "ル・アビタウ神殿", mob = "Zazalda Clot" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [213] = {
        name = "バーストII",
        description = "敵に雷属性のダメージを与え、土属性に対する防御力をダウン。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [214] = {
        name = "フラッド",
        description = "敵に水属性のダメージを与え、雷属性に対する防御力をダウン。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {
            { zone = "ベドー", mob = "Yagudo Sentinel" },
            { zone = "クゥルンの大伽藍", mob = "Zeid" },
            { zone = "怨念洞", mob = "Yagudo Sentinel" },
            { zone = "怨念洞", mob = "Yearner" },
            { zone = "ベドー〔Ｓ〕", mob = "Young Quadav" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "戦績BF ひとまねことり\n戦績BF ローリングバグズ\n戦績BF タラッカの笛吹き",
        acquisition = "",
    },
    [215] = {
        name = "フラッドII",
        description = "敵に水属性のダメージを与え、雷属性に対する防御力をダウン。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [216] = {
        name = "グラビデ",
        description = "敵をヘヴィ状態にして、移動速度を遅くする。",
        shop = "",
        quest = "",
        drop = {
            { zone = "グスゲン鉱山", mob = "Yagudo's Elemental" },
            { zone = "グスゲン鉱山", mob = "Zjaa Bao the Wrathherald" },
            { zone = "ギルド浅橋", mob = "Yagudo Sentinel" },
            { zone = "内ホルトト遺跡", mob = "Zhayolm Apkallu" },
            { zone = "外ホルトト遺跡", mob = "Zerde's Haupia" },
            { zone = "外ホルトト遺跡", mob = "Zoredonite" },
            { zone = "シャクラミの地下迷宮", mob = "Ziz" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [220] = {
        name = "ポイズン",
        description = "敵は毒で徐々にHPを失う。",
        shop = "バストゥーク商業区 H-5T1 Zaira\nウィンダス港 H-8T8 Aroro",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [221] = {
        name = "ポイズンII",
        description = "敵は毒で徐々にHPを失う。",
        shop = "ウィンダス水の区 南 G-7T3 Hilkomu-Makimu\nアトルガン白門 G-7T7 Mazween\nセルビナ H-9T5 Chutarmire",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [225] = {
        name = "ポイゾガ",
        description = "範囲内の敵は毒で徐々にHPを失う。",
        shop = "ウィンダス水の区 南 G-7T3 Hilkomu-Makimu\nアトルガン白門 G-7T7 Mazween\nセルビナ H-9T5 Chutarmire",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [230] = {
        name = "バイオ",
        description = "敵を闇属性のダメージがじわじわ蝕み攻撃力ダウン。",
        shop = "バストゥーク商業区 H-5T1 Zaira\nウィンダス港 H-8T8 Aroro",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [231] = {
        name = "バイオII",
        description = "敵を闇属性のダメージがじわじわ蝕み攻撃力ダウン。",
        shop = "ウィンダス水の区 南 G-7T3 Hilkomu-Makimu\nアトルガン白門 G-7T7 Mazween\nセルビナ H-9T5 Chutarmire",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [232] = {
        name = "バイオIII",
        description = "敵を闇属性のダメージがじわじわ蝕み攻撃力ダウン。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [235] = {
        name = "バーン",
        description = "敵を火属性のダメージがじわじわ蝕みINTダウン。",
        shop = "バストゥーク商業区 H-5T1 Zaira\nウィンダス港 H-8T8 Aroro",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [236] = {
        name = "フロスト",
        description = "敵を氷属性のダメージがじわじわ蝕みAGIダウン。",
        shop = "バストゥーク商業区 H-5T1 Zaira\nウィンダス港 H-8T8 Aroro",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [237] = {
        name = "チョーク",
        description = "敵を風属性のダメージがじわじわ蝕みVITダウン。",
        shop = "バストゥーク商業区 H-5T1 Zaira\nウィンダス港 H-8T8 Aroro",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [238] = {
        name = "ラスプ",
        description = "敵を土属性のダメージがじわじわ蝕みDEXダウン。",
        shop = "バストゥーク商業区 H-5T1 Zaira\nウィンダス港 H-8T8 Aroro",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [239] = {
        name = "ショック",
        description = "敵を雷属性のダメージがじわじわ蝕みMNDダウン。",
        shop = "バストゥーク商業区 H-5T1 Zaira\nウィンダス港 H-8T8 Aroro",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [240] = {
        name = "ドラウン",
        description = "敵を水属性のダメージがじわじわ蝕みSTRダウン。",
        shop = "バストゥーク商業区 H-5T1 Zaira\nウィンダス港 H-8T8 Aroro",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [286] = {
        name = "アドル",
        description = "敵の魔法命中率をダウンし、詠唱時間を長くする。",
        shop = "ジュノ下層 H-9T2 Creepstix\nジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [473] = {
        name = "リフレシュII",
        description = "標的のパーティメンバーのMPを徐々に回復する。",
        shop = "ル・ルデの庭 H-9T1 Macchi Gazlitah",
        quest = "",
        drop = {
            { zone = "ラングモント峠", mob = "Ziz" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [245] = {
        name = "ドレイン",
        description = "敵のHPを吸収する。不死生物には効かない。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "サンドリア 地下牢番人の噂",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF クラウディオーブ 悩殺三姉妹",
        acquisition = "",
    },
    [246] = {
        name = "ドレインII",
        description = "敵のHPを吸収し、場合によってHPmaxアップ。不死生物には効かない。",
        shop = "アトルガン白門 G-7T7 Mazween",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [247] = {
        name = "アスピル",
        description = "敵のMPを吸収する。不死生物には効かない。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "ウィンダス 間違いと過ち",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [248] = {
        name = "アスピルII",
        description = "敵のMPを吸収する。不死生物には効かない。",
        shop = "アトルガン白門 G-7T7 Mazween",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [249] = {
        name = "ブレイズスパイク",
        description = "火属性のスパイクアーマーで自身をつつむ。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "ウィンダス オバケの家",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF クラウディオーブ 恐怖の狙撃手",
        acquisition = "",
    },
    [250] = {
        name = "アイススパイク",
        description = "氷属性のスパイクアーマーで自身をつつむ。",
        shop = "",
        quest = "",
        drop = {
            { zone = "機船航路セルビナ行き", mob = "Zircon Quadav" },
            { zone = "機船航路マウラ行き", mob = "Zircon Quadav" },
            { zone = "機船航路セルビナ行き（海賊）", mob = "Zircon Quadav" },
            { zone = "機船航路マウラ行き（海賊）", mob = "Zircon Quadav" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF スターオーブ 上様御一行\n印章BF スターオーブ 居候妖精\n印章BF スターオーブ 地獄の尻尾\n印章BF スターオーブ 光る眼\n印章BF スターオーブ 人形工房\n印章BF スターオーブ 生ける屍\n印章BF スターオーブ 噂のスライム\n印章BF スターオーブ 地竜大王",
        acquisition = "",
    },
    [251] = {
        name = "ショックスパイク",
        description = "雷属性のスパイクアーマーで自身をつつむ。",
        shop = "ウィンダス水の区 南 G-7T3 Hilkomu-Makimu\nセルビナ H-9T5 Chutarmire\nナシュモ H-7T6 Mamaroon",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [252] = {
        name = "スタン",
        description = "敵を短時間行動できなくする。",
        shop = "ラバオ F-7T8 Brave Ox\nナシュモ H-7T6 Mamaroon",
        quest = "",
        drop = {
            { zone = "エルディーム古墳", mob = "Yagudo Scribe" },
            { zone = "エルディーム古墳", mob = "Yagudo's Elemental" },
            { zone = "エルディーム古墳", mob = "Ziryu" },
            { zone = "エルディーム古墳", mob = "Zombie" },
            { zone = "エルディーム古墳", mob = "Zoredonite" },
            { zone = "ダボイ", mob = "Zuu Xowu the Darksmoke" },
            { zone = "修道窟", mob = "Yagudo Scribe" },
            { zone = "修道窟", mob = "Ziz" },
            { zone = "オンゾゾの迷宮", mob = "Zeid" },
            { zone = "ガルレージュ要塞", mob = "Yilbegan" },
            { zone = "ガルレージュ要塞", mob = "Zoredonite" },
            { zone = "ズヴァール城外郭", mob = "Zombie" },
            { zone = "ズヴァール城内郭", mob = "Zoraal Ja's Pkuucha" },
            { zone = "ズヴァール城内郭", mob = "Zoredonite" },
            { zone = "ベヒーモスの縄張り", mob = "Ziz" },
            { zone = "ベヒーモスの縄張り", mob = "Zymotic Zuulie" },
            { zone = "聖地ジ・タ", mob = "Yowie" },
            { zone = "西アルテパ砂漠", mob = "Ziryu" },
            { zone = "グスタフの洞門", mob = "Zu" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [253] = {
        name = "スリプル",
        description = "敵を眠らせる。",
        shop = "バストゥーク商業区 H-5T1 Zaira\nウィンダス港 H-8T8 Aroro",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [254] = {
        name = "ブライン",
        description = "敵を暗闇で捕らえ、命中率ダウン。",
        shop = "バストゥーク商業区 H-5T1 Zaira\nウィンダス港 H-8T8 Aroro",
        quest = "",
        drop = {
            { zone = "西ロンフォール", mob = "Zombie" },
            { zone = "東ロンフォール", mob = "Ziz" },
            { zone = "ゲルスバ野営陣", mob = "Zombie" },
            { zone = "ゲルスバ砦", mob = "Ziz" },
            { zone = "コンシュタット高地", mob = "Zombie" },
            { zone = "ラテーヌ高原", mob = "Yhoator Wasp" },
            { zone = "北グスタベルグ", mob = "Ze'Vho Fallsplitter" },
            { zone = "南グスタベルグ", mob = "Yearner" },
            { zone = "パルブロ鉱山", mob = "Ziz" },
            { zone = "西サルタバルタ", mob = "Zymotic Zuulie" },
            { zone = "東サルタバルタ", mob = "Yagudo Scribe" },
            { zone = "ギデアス", mob = "Zircon Quadav" },
            { zone = "タロンギ大峡谷", mob = "Zurko-Bazurko" },
            { zone = "グロウベルグ〔Ｓ〕", mob = "Zombie" },
            { zone = "パシュハウ沼〔Ｓ〕", mob = "Zurko-Bazurko" },
            { zone = "ロランベリー耕地〔Ｓ〕", mob = "Yoo Mihi the Haze" },
            { zone = "ベドー〔Ｓ〕", mob = "Ziz" },
            { zone = "クロウラーの巣〔Ｓ〕", mob = "Zircon Quadav" },
            { zone = "ズヴァール城外郭〔Ｓ〕", mob = "Zizzy Zillah" },
            { zone = "ズヴァール城内郭〔Ｓ〕", mob = "Ziz" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [255] = {
        name = "ブレイク",
        description = "敵を硬直させて行動できなくする。",
        shop = "ウィンダス水の区〔Ｓ〕 南 H-8T7 Ezura-Romazura",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [258] = {
        name = "バインド",
        description = "敵を移動できなくする。",
        shop = "",
        quest = "",
        drop = {
            { zone = "西ロンフォール", mob = "Zombie" },
            { zone = "東ロンフォール", mob = "Ziz" },
            { zone = "ゲルスバ野営陣", mob = "Zombie" },
            { zone = "ゲルスバ砦", mob = "Ziz" },
            { zone = "コンシュタット高地", mob = "Zombie" },
            { zone = "ラテーヌ高原", mob = "Yhoator Wasp" },
            { zone = "北グスタベルグ", mob = "Ze'Vho Fallsplitter" },
            { zone = "南グスタベルグ", mob = "Yearner" },
            { zone = "パルブロ鉱山", mob = "Ziz" },
            { zone = "西サルタバルタ", mob = "Zymotic Zuulie" },
            { zone = "東サルタバルタ", mob = "Yagudo Scribe" },
            { zone = "ギデアス", mob = "Zircon Quadav" },
            { zone = "タロンギ大峡谷", mob = "Zurko-Bazurko" },
            { zone = "グロウベルグ〔Ｓ〕", mob = "Zombie" },
            { zone = "パシュハウ沼〔Ｓ〕", mob = "Zurko-Bazurko" },
            { zone = "ロランベリー耕地〔Ｓ〕", mob = "Yoo Mihi the Haze" },
            { zone = "ベドー〔Ｓ〕", mob = "Ziz" },
            { zone = "クロウラーの巣〔Ｓ〕", mob = "Zircon Quadav" },
            { zone = "ズヴァール城外郭〔Ｓ〕", mob = "Zizzy Zillah" },
            { zone = "ズヴァール城内郭〔Ｓ〕", mob = "Ziz" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [259] = {
        name = "スリプルII",
        description = "敵を眠らせる。",
        shop = "ジュノ下層 H-9T9 Susu\nアトルガン白門 G-7T7 Mazween",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [260] = {
        name = "ディスペル",
        description = "敵の魔法効果を1つ消し去る。",
        shop = "ウィンダス水の区 南 G-7T3 Shohrun-Tuhrun\nアルザビ J-8T5 Zafif\nラバオ F-7T8 Brave Ox",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF スカイオーブ 断食芋虫\n印章BF スカイオーブ 天鳴地響\n印章BF スカイオーブ 石竜の双璧\n印章BF スカイオーブ 山岳甲兵\n印章BF スカイオーブ 降下猟兵\n戦績BF プークの幻影\n戦績BF 詠唱中は別の顔\n印章BF スカイオーブ 剣闘士の得物\n印章BF スカイオーブ 凶狗四天王\n戦績BF オロボンの呪縛\n印章BF スカイオーブ 飛行小隊\n印章BF スカイオーブ 森の乙女",
        acquisition = "",
    },
    [261] = {
        name = "デジョン",
        description = "ホームポイントへワープする。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "バストゥーク ゴキゲンナナメな機械",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF クラウディオーブ 恐怖の狙撃手",
        acquisition = "",
    },
    [262] = {
        name = "デジョンII",
        description = "標的のパーティメンバーをホームポイントへワープさせる。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "ウィンダス ゴーレムのまなざし",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [263] = {
        name = "エスケプ",
        description = "範囲内のパーティメンバーをダンジョンから脱出させる。",
        shop = "",
        quest = "",
        drop = {
            { zone = "グスゲン鉱山", mob = "Zhu Que" },
            { zone = "クフィム島", mob = "Zoraal Ja's Pkuucha" },
            { zone = "デルクフの塔中層", mob = "Zonpa-Zippa" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [264] = {
        name = "トラクタ",
        description = "戦闘不能の標的を自分の足下に引き寄せる。",
        shop = "",
        quest = "",
        drop = {
            { zone = "グスゲン鉱山", mob = "Yilbegan" },
            { zone = "グスゲン鉱山", mob = "Ziryu" },
            { zone = "バタリア丘陵", mob = "Wight" },
            { zone = "バタリア丘陵", mob = "Ziryu" },
            { zone = "ロランベリー耕地", mob = "Zolku-Azolku" },
            { zone = "ロランベリー耕地", mob = "Zymotic Zuulie" },
            { zone = "シャクラミの地下迷宮", mob = "Yilbegan" },
            { zone = "シャクラミの地下迷宮", mob = "Zo'Dhu Legslicer" },
            { zone = "ソロムグ原野", mob = "Zurmurwur the Ruthless" },
            { zone = "ソロムグ原野", mob = "Zymotic Zuulie" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [241] = {
        name = "リトレース",
        description = "標的の資格があるパーティメンバーを過去の所属国へワープさせる。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "過去世界 失われた書",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [266] = {
        name = "アブゾースト",
        description = "敵のSTRを吸収する。",
        shop = "バストゥーク港 F-6T5 Jabbar",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF スターオーブ 上様御一行\n印章BF スターオーブ 居候妖精\n印章BF ムーンオーブ 第11軍団独立支隊\n印章BF スターオーブ 地獄の尻尾\n印章BF スターオーブ 光る眼\n印章BF スターオーブ 人形工房\n印章BF スターオーブ 生ける屍\n印章BF ムーンオーブ 潜行特務隊\n印章BF ムーンオーブ 特命介錯人\n印章BF スターオーブ 噂のスライム\n印章BF スターオーブ 地竜大王",
        acquisition = "",
    },
    [267] = {
        name = "アブゾデック",
        description = "敵のDEXを吸収する。",
        shop = "",
        quest = "",
        drop = {
            { zone = "ズヴァール城外郭", mob = "Yilbegan" },
            { zone = "ズヴァール城内郭", mob = "Zo'Bha Helmkeeper" },
            { zone = "ウルガラン山脈", mob = "Ziryu" },
            { zone = "ザルカバード〔Ｓ〕", mob = "Zymotic Zuulie" },
            { zone = "ズヴァール城内郭〔Ｓ〕", mob = "Zoldeff Jagil" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [268] = {
        name = "アブゾバイト",
        description = "敵のVITを吸収する。",
        shop = "バストゥーク港 F-6T5 Jabbar\nノーグ I-8T1 Chiyo",
        quest = "",
        drop = {
            { zone = "グスゲン鉱山", mob = "Yilbegan" },
            { zone = "グスゲン鉱山", mob = "Ziryu" },
            { zone = "エルディーム古墳", mob = "Yagudo Scribe" },
            { zone = "エルディーム古墳", mob = "Yagudo's Elemental" },
            { zone = "エルディーム古墳", mob = "Yara Ma Yha Who" },
            { zone = "エルディーム古墳", mob = "Yearner" },
            { zone = "エルディーム古墳", mob = "Yoo Mihi the Haze" },
            { zone = "エルディーム古墳", mob = "Zeid" },
            { zone = "エルディーム古墳", mob = "Zhayolm Apkallu" },
            { zone = "エルディーム古墳", mob = "Zhu Que" },
            { zone = "エルディーム古墳", mob = "Ziryu" },
            { zone = "エルディーム古墳", mob = "Zo'Dhu Legslicer" },
            { zone = "エルディーム古墳", mob = "Zombie" },
            { zone = "エルディーム古墳", mob = "Zoredonite" },
            { zone = "ダボイ", mob = "Zizzy Zillah" },
            { zone = "ダボイ", mob = "Zombie" },
            { zone = "ダボイ", mob = "Zuu Xowu the Darksmoke" },
            { zone = "修道窟", mob = "Ziz" },
            { zone = "ベドー", mob = "Zo'Dhu Legslicer" },
            { zone = "トライマライ水路", mob = "Zircon Quadav" },
            { zone = "シャクラミの地下迷宮", mob = "Yilbegan" },
            { zone = "シャクラミの地下迷宮", mob = "Zo'Dhu Legslicer" },
            { zone = "オンゾゾの迷宮", mob = "Zeid" },
            { zone = "ガルレージュ要塞", mob = "Yilbegan" },
            { zone = "ガルレージュ要塞", mob = "Zeid" },
            { zone = "ガルレージュ要塞", mob = "Ziryu" },
            { zone = "ガルレージュ要塞", mob = "Zoredonite" },
            { zone = "ガルレージュ要塞", mob = "Zvahl Fortalice" },
            { zone = "アットワ地溝", mob = "Zonpa-Zippa" },
            { zone = "ボスディン氷河", mob = "Ziryu" },
            { zone = "ボスディン氷河", mob = "Zo'Dhu Legslicer" },
            { zone = "ボスディン氷河", mob = "Zoraal Ja's Pkuucha" },
            { zone = "フェ・イン", mob = "Yearner" },
            { zone = "フェ・イン", mob = "Zeid" },
            { zone = "フェ・イン", mob = "Zhayolm Apkallu" },
            { zone = "フェ・イン", mob = "Zurko-Bazurko" },
            { zone = "ザルカバード", mob = "Yagudo Votary" },
            { zone = "ザルカバード", mob = "Zazalda Clot" },
            { zone = "ザルカバード", mob = "Zizzy Zillah" },
            { zone = "ズヴァール城外郭", mob = "Zirnitra" },
            { zone = "ズヴァール城外郭", mob = "Zoldeff Jagil" },
            { zone = "ズヴァール城外郭", mob = "Zombie" },
            { zone = "ズヴァール城内郭", mob = "Zeid" },
            { zone = "ズヴァール城内郭", mob = "Zo'Dhu Legslicer" },
            { zone = "ズヴァール城内郭", mob = "Zoraal Ja's Pkuucha" },
            { zone = "ズヴァール城内郭", mob = "Zoredonite" },
            { zone = "ウルガラン山脈", mob = "Young Quadav" },
            { zone = "聖地ジ・タ", mob = "Yowie" },
            { zone = "聖地ジ・タ", mob = "Zuu Xowu the Darksmoke" },
            { zone = "東アルテパ砂漠", mob = "Zolku-Azolku" },
            { zone = "西アルテパ砂漠", mob = "Ziryu" },
            { zone = "グスタフの洞門", mob = "Zu" },
            { zone = "ユタンガ大森林", mob = "Zuu Xowu the Darksmoke" },
            { zone = "海蛇の岩窟", mob = "Zagan's Wyvern" },
            { zone = "海蛇の岩窟", mob = "Zoredonite" },
            { zone = "ヨアトル大森林", mob = "Ze'Vho Fallsplitter" },
            { zone = "東ロンフォール〔Ｓ〕", mob = "Young Quadav" },
            { zone = "ジャグナー森林〔Ｓ〕", mob = "Ghoul" },
            { zone = "ジャグナー森林〔Ｓ〕", mob = "Zonpa-Zippa" },
            { zone = "北グスタベルグ〔Ｓ〕", mob = "Ziz" },
            { zone = "ロランベリー耕地〔Ｓ〕", mob = "Young Quadav" },
            { zone = "西サルタバルタ〔Ｓ〕", mob = "Zymotic Zuulie" },
            { zone = "アビセア-アットワ", mob = "Yagudo's Elemental" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF スカイオーブ 石竜の双璧\n印章BF スカイオーブ 凶狗四天王",
        acquisition = "",
    },
    [269] = {
        name = "アブゾアジル",
        description = "敵のAGIを吸収する。",
        shop = "バストゥーク港 F-6T5 Jabbar\nノーグ I-8T1 Chiyo",
        quest = "",
        drop = {
            { zone = "グスゲン鉱山", mob = "Yilbegan" },
            { zone = "グスゲン鉱山", mob = "Ziryu" },
            { zone = "エルディーム古墳", mob = "Yagudo Scribe" },
            { zone = "エルディーム古墳", mob = "Yagudo's Elemental" },
            { zone = "エルディーム古墳", mob = "Yara Ma Yha Who" },
            { zone = "エルディーム古墳", mob = "Yearner" },
            { zone = "エルディーム古墳", mob = "Yoo Mihi the Haze" },
            { zone = "エルディーム古墳", mob = "Zeid" },
            { zone = "エルディーム古墳", mob = "Zhayolm Apkallu" },
            { zone = "エルディーム古墳", mob = "Zhu Que" },
            { zone = "エルディーム古墳", mob = "Ziryu" },
            { zone = "エルディーム古墳", mob = "Zo'Dhu Legslicer" },
            { zone = "エルディーム古墳", mob = "Zombie" },
            { zone = "エルディーム古墳", mob = "Zoredonite" },
            { zone = "ダボイ", mob = "Zizzy Zillah" },
            { zone = "ダボイ", mob = "Zombie" },
            { zone = "ダボイ", mob = "Zuu Xowu the Darksmoke" },
            { zone = "修道窟", mob = "Ziz" },
            { zone = "ベドー", mob = "Zo'Dhu Legslicer" },
            { zone = "トライマライ水路", mob = "Zircon Quadav" },
            { zone = "シャクラミの地下迷宮", mob = "Yilbegan" },
            { zone = "シャクラミの地下迷宮", mob = "Zo'Dhu Legslicer" },
            { zone = "オンゾゾの迷宮", mob = "Zeid" },
            { zone = "ガルレージュ要塞", mob = "Yilbegan" },
            { zone = "ガルレージュ要塞", mob = "Zeid" },
            { zone = "ガルレージュ要塞", mob = "Ziryu" },
            { zone = "ガルレージュ要塞", mob = "Zoredonite" },
            { zone = "ガルレージュ要塞", mob = "Zvahl Fortalice" },
            { zone = "アットワ地溝", mob = "Zonpa-Zippa" },
            { zone = "ボスディン氷河", mob = "Ziryu" },
            { zone = "ボスディン氷河", mob = "Zo'Dhu Legslicer" },
            { zone = "ボスディン氷河", mob = "Zoraal Ja's Pkuucha" },
            { zone = "フェ・イン", mob = "Yearner" },
            { zone = "フェ・イン", mob = "Zeid" },
            { zone = "フェ・イン", mob = "Zhayolm Apkallu" },
            { zone = "フェ・イン", mob = "Zurko-Bazurko" },
            { zone = "ザルカバード", mob = "Yagudo Votary" },
            { zone = "ザルカバード", mob = "Zazalda Clot" },
            { zone = "ザルカバード", mob = "Zizzy Zillah" },
            { zone = "ズヴァール城外郭", mob = "Zirnitra" },
            { zone = "ズヴァール城外郭", mob = "Zoldeff Jagil" },
            { zone = "ズヴァール城外郭", mob = "Zombie" },
            { zone = "ズヴァール城内郭", mob = "Zeid" },
            { zone = "ズヴァール城内郭", mob = "Zo'Dhu Legslicer" },
            { zone = "ズヴァール城内郭", mob = "Zoraal Ja's Pkuucha" },
            { zone = "ズヴァール城内郭", mob = "Zoredonite" },
            { zone = "ウルガラン山脈", mob = "Young Quadav" },
            { zone = "聖地ジ・タ", mob = "Yowie" },
            { zone = "聖地ジ・タ", mob = "Zuu Xowu the Darksmoke" },
            { zone = "東アルテパ砂漠", mob = "Zolku-Azolku" },
            { zone = "西アルテパ砂漠", mob = "Ziryu" },
            { zone = "グスタフの洞門", mob = "Zu" },
            { zone = "ユタンガ大森林", mob = "Zuu Xowu the Darksmoke" },
            { zone = "海蛇の岩窟", mob = "Zagan's Wyvern" },
            { zone = "海蛇の岩窟", mob = "Zoredonite" },
            { zone = "ヨアトル大森林", mob = "Ze'Vho Fallsplitter" },
            { zone = "東ロンフォール〔Ｓ〕", mob = "Young Quadav" },
            { zone = "ジャグナー森林〔Ｓ〕", mob = "Ghoul" },
            { zone = "ジャグナー森林〔Ｓ〕", mob = "Zonpa-Zippa" },
            { zone = "北グスタベルグ〔Ｓ〕", mob = "Ziz" },
            { zone = "ロランベリー耕地〔Ｓ〕", mob = "Young Quadav" },
            { zone = "西サルタバルタ〔Ｓ〕", mob = "Zymotic Zuulie" },
            { zone = "アビセア-アットワ", mob = "Yagudo's Elemental" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF スカイオーブ 石竜の双璧\n印章BF スカイオーブ 山岳甲兵\n印章BF スカイオーブ 凶狗四天王",
        acquisition = "",
    },
    [270] = {
        name = "アブゾイン",
        description = "敵のINTを吸収する。",
        shop = "バストゥーク港 F-6T5 Jabbar\nノーグ I-8T1 Chiyo",
        quest = "",
        drop = {
            { zone = "グスゲン鉱山", mob = "Yilbegan" },
            { zone = "グスゲン鉱山", mob = "Ziryu" },
            { zone = "エルディーム古墳", mob = "Yagudo Scribe" },
            { zone = "エルディーム古墳", mob = "Yagudo's Elemental" },
            { zone = "エルディーム古墳", mob = "Yara Ma Yha Who" },
            { zone = "エルディーム古墳", mob = "Yearner" },
            { zone = "エルディーム古墳", mob = "Yoo Mihi the Haze" },
            { zone = "エルディーム古墳", mob = "Zeid" },
            { zone = "エルディーム古墳", mob = "Zhayolm Apkallu" },
            { zone = "エルディーム古墳", mob = "Zhu Que" },
            { zone = "エルディーム古墳", mob = "Ziryu" },
            { zone = "エルディーム古墳", mob = "Zo'Dhu Legslicer" },
            { zone = "エルディーム古墳", mob = "Zombie" },
            { zone = "エルディーム古墳", mob = "Zoredonite" },
            { zone = "ダボイ", mob = "Zizzy Zillah" },
            { zone = "ダボイ", mob = "Zombie" },
            { zone = "ダボイ", mob = "Zuu Xowu the Darksmoke" },
            { zone = "修道窟", mob = "Ziz" },
            { zone = "ベドー", mob = "Zo'Dhu Legslicer" },
            { zone = "トライマライ水路", mob = "Zircon Quadav" },
            { zone = "シャクラミの地下迷宮", mob = "Yilbegan" },
            { zone = "シャクラミの地下迷宮", mob = "Zo'Dhu Legslicer" },
            { zone = "オンゾゾの迷宮", mob = "Zeid" },
            { zone = "ガルレージュ要塞", mob = "Yilbegan" },
            { zone = "ガルレージュ要塞", mob = "Zeid" },
            { zone = "ガルレージュ要塞", mob = "Ziryu" },
            { zone = "ガルレージュ要塞", mob = "Zoredonite" },
            { zone = "ガルレージュ要塞", mob = "Zvahl Fortalice" },
            { zone = "アットワ地溝", mob = "Zonpa-Zippa" },
            { zone = "ボスディン氷河", mob = "Ziryu" },
            { zone = "ボスディン氷河", mob = "Zo'Dhu Legslicer" },
            { zone = "ボスディン氷河", mob = "Zoraal Ja's Pkuucha" },
            { zone = "フェ・イン", mob = "Yearner" },
            { zone = "フェ・イン", mob = "Zeid" },
            { zone = "フェ・イン", mob = "Zhayolm Apkallu" },
            { zone = "フェ・イン", mob = "Zurko-Bazurko" },
            { zone = "ザルカバード", mob = "Yagudo Votary" },
            { zone = "ザルカバード", mob = "Zazalda Clot" },
            { zone = "ザルカバード", mob = "Zizzy Zillah" },
            { zone = "ズヴァール城外郭", mob = "Zirnitra" },
            { zone = "ズヴァール城外郭", mob = "Zoldeff Jagil" },
            { zone = "ズヴァール城外郭", mob = "Zombie" },
            { zone = "ズヴァール城内郭", mob = "Zeid" },
            { zone = "ズヴァール城内郭", mob = "Zo'Dhu Legslicer" },
            { zone = "ズヴァール城内郭", mob = "Zoraal Ja's Pkuucha" },
            { zone = "ズヴァール城内郭", mob = "Zoredonite" },
            { zone = "ウルガラン山脈", mob = "Young Quadav" },
            { zone = "聖地ジ・タ", mob = "Yowie" },
            { zone = "聖地ジ・タ", mob = "Zuu Xowu the Darksmoke" },
            { zone = "東アルテパ砂漠", mob = "Zolku-Azolku" },
            { zone = "西アルテパ砂漠", mob = "Ziryu" },
            { zone = "グスタフの洞門", mob = "Zu" },
            { zone = "ユタンガ大森林", mob = "Zuu Xowu the Darksmoke" },
            { zone = "海蛇の岩窟", mob = "Zagan's Wyvern" },
            { zone = "海蛇の岩窟", mob = "Zoredonite" },
            { zone = "ヨアトル大森林", mob = "Ze'Vho Fallsplitter" },
            { zone = "東ロンフォール〔Ｓ〕", mob = "Young Quadav" },
            { zone = "ジャグナー森林〔Ｓ〕", mob = "Ghoul" },
            { zone = "ジャグナー森林〔Ｓ〕", mob = "Zonpa-Zippa" },
            { zone = "北グスタベルグ〔Ｓ〕", mob = "Ziz" },
            { zone = "ロランベリー耕地〔Ｓ〕", mob = "Young Quadav" },
            { zone = "西サルタバルタ〔Ｓ〕", mob = "Zymotic Zuulie" },
            { zone = "アビセア-アットワ", mob = "Yagudo's Elemental" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF スカイオーブ 石竜の双璧\n印章BF スカイオーブ 山岳甲兵\n印章BF スカイオーブ 凶狗四天王",
        acquisition = "",
    },
    [271] = {
        name = "アブゾマイン",
        description = "敵のMNDを吸収する。",
        shop = "バストゥーク港 F-6T5 Jabbar\nノーグ I-8T1 Chiyo",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [272] = {
        name = "アブゾカリス",
        description = "敵のCHRを吸収する。",
        shop = "バストゥーク港 F-6T5 Jabbar\nノーグ I-8T1 Chiyo",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [273] = {
        name = "スリプガ",
        description = "範囲内の敵を眠らせる。",
        shop = "ジュノ下層 H-9T9 Susu\nアトルガン白門 G-7T7 Mazween\nマウラ G-9T1 Tya Padolih",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [274] = {
        name = "スリプガII",
        description = "範囲内の敵を眠らせる。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [275] = {
        name = "アブゾタック",
        description = "敵のTPを吸収する。",
        shop = "アトルガン白門 G-7T7 Mazween",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [276] = {
        name = "ブラインII",
        description = "敵を暗闇で捕らえ、命中率ダウン。",
        shop = "ジュノ下層 H-9T9 Susu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [277] = {
        name = "ドレッドスパイク",
        description = "闇属性のスパイクアーマーで自身をつつみ、反撃効果:HP吸収。不死生物には効かない。",
        shop = "アトルガン白門 G-7T7 Mazween",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [242] = {
        name = "アブゾアキュル",
        description = "敵の命中を吸収する。",
        shop = "アトルガン白門 G-7T7 Mazween",
        quest = "",
        drop = {
            { zone = "東ロンフォール〔Ｓ〕", mob = "Yagudo Sentinel" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [496] = {
        name = "ファイジャ",
        description = "範囲内の敵に火属性のダメージを与え、さらに重ねて使用すると効果アップ。",
        shop = "ウィンダス水の区〔Ｓ〕 南 H-8T7 Ezura-Romazura",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [498] = {
        name = "エアロジャ",
        description = "範囲内の敵に風属性のダメージを与え、さらに重ねて使用すると効果アップ。",
        shop = "ウィンダス水の区〔Ｓ〕 南 H-8T7 Ezura-Romazura",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [499] = {
        name = "ストンジャ",
        description = "範囲内の敵に土属性のダメージを与え、さらに重ねて使用すると効果アップ。",
        shop = "ウィンダス水の区〔Ｓ〕 南 H-8T7 Ezura-Romazura",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [501] = {
        name = "ウォタジャ",
        description = "範囲内の敵に水属性のダメージを与え、さらに重ねて使用すると効果アップ。",
        shop = "ウィンダス水の区〔Ｓ〕 南 H-8T7 Ezura-Romazura",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [288] = {
        name = "火精霊召喚",
        description = "火の精霊を召喚する。",
        shop = "",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF コメットオーブ 爆走爬竜\n印章BF スターオーブ 上様御一行\n印章BF スターオーブ 居候妖精\n印章BF ムーンオーブ 第11軍団独立支隊\n印章BF コメットオーブ 虎視眈眈\n印章BF スターオーブ 地獄の尻尾\n印章BF スターオーブ 光る眼\n印章BF コメットオーブ 餓鬼武者\n印章BF スターオーブ 人形工房\n印章BF コメットオーブ 四界の神像\n印章BF スターオーブ 生ける屍\n印章BF ムーンオーブ 潜行特務隊\n印章BF ムーンオーブ 特命介錯人\n印章BF コメットオーブ 刻一刻\n印章BF スターオーブ 噂のスライム\n印章BF スターオーブ 地竜大王",
        acquisition = "",
    },
    [289] = {
        name = "氷精霊召喚",
        description = "氷の精霊を召喚する。",
        shop = "",
        quest = "",
        drop = {
            { zone = "ズヴァール城外郭", mob = "Yagudo Sentinel" },
            { zone = "ズヴァール城外郭", mob = "Zo'Bha Helmkeeper" },
            { zone = "ズヴァール城外郭", mob = "Zombie" },
            { zone = "ズヴァール城外郭", mob = "Zuu Xowu the Darksmoke" },
            { zone = "ズヴァール城内郭", mob = "Zoldeff Jagil" },
            { zone = "ウルガラン山脈", mob = "Yearner" },
            { zone = "ザルカバード〔Ｓ〕", mob = "Young Quadav" },
            { zone = "ズヴァール城内郭〔Ｓ〕", mob = "Za'Dha's Biographer" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF ムーンオーブ 蒼の血族",
        acquisition = "",
    },
    [290] = {
        name = "風精霊召喚",
        description = "風の精霊を召喚する。",
        shop = "",
        quest = "",
        drop = {
            { zone = "ソロムグ原野", mob = "Yagudo's Elemental" },
            { zone = "オズトロヤ城", mob = "Za'Dha Adamantking" },
            { zone = "オズトロヤ城", mob = "Ziz" },
            { zone = "西サルタバルタ〔Ｓ〕", mob = "Ziryu" },
            { zone = "メリファト山地〔Ｓ〕", mob = "Zazalda Clot" },
            { zone = "ソロムグ原野〔Ｓ〕", mob = "Zhayolm Apkallu" },
            { zone = "オズトロヤ城〔Ｓ〕", mob = "Zircon Quadav" },
            { zone = "オズトロヤ城〔Ｓ〕", mob = "Zirnitra" },
            { zone = "ガルレージュ要塞〔Ｓ〕", mob = "Yhoator Wasp" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF クラウディオーブ 悩殺三姉妹",
        acquisition = "",
    },
    [291] = {
        name = "土精霊召喚",
        description = "土の精霊を召喚する。",
        shop = "ノーグ H-9T4 Solby-Maholby",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [292] = {
        name = "雷精霊召喚",
        description = "雷の精霊を召喚する。",
        shop = "",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF クラウディオーブ 闇夜の舞姫\n印章BF クラウディオーブ 恐怖の狙撃手",
        acquisition = "",
    },
    [293] = {
        name = "水精霊召喚",
        description = "水の精霊を召喚する。",
        shop = "",
        quest = "",
        drop = {
            { zone = "ソ・ジヤ", mob = "Yearner" },
            { zone = "ヨアトル大森林", mob = "Yagudo Sentinel" },
            { zone = "ヨアトル大森林", mob = "Yrvaulair S Cousseraux" },
            { zone = "ウガレピ寺院", mob = "Zizzy Zillah" },
            { zone = "ウガレピ寺院", mob = "Zolku-Azolku" },
            { zone = "ウガレピ寺院", mob = "Zuu Xowu the Darksmoke" },
            { zone = "怨念洞", mob = "Yagudo's Elemental" },
            { zone = "怨念洞", mob = "Young Quadav" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [294] = {
        name = "光精霊召喚",
        description = "光の精霊を召喚する。",
        shop = "",
        quest = "",
        drop = {
            { zone = "ウガレピ寺院", mob = "Yilbegan" },
            { zone = "ウガレピ寺院", mob = "Zuu Xowu the Darksmoke" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF コメットオーブ 爆走爬竜\n印章BF コメットオーブ 三つの葛籠\n印章BF クローソーオーブ 伝説のチャンピオン\n印章BF ラキシスオーブ 飛竜の眷族\n印章BF アトロポスオーブ 紅蓮の翼\n印章BF コメットオーブ 虎視眈眈\n印章BF コメットオーブ 戦う羊\n戦績BF ひとまねことり\n戦績BF ローリングバグズ\n印章BF コメットオーブ 餓鬼武者\n印章BF クローソーオーブ 蜘蛛の神\n印章BF ラキシスオーブ 破滅の壺\n印章BF コメットオーブ 四界の神像\n印章BF アトロポスオーブ 蝿の王\n戦績BF タラッカの笛吹き\n印章BF コメットオーブ 刻一刻",
        acquisition = "",
    },
    [295] = {
        name = "闇精霊召喚",
        description = "闇の精霊を召喚する。",
        shop = "",
        quest = "",
        drop = {
            { zone = "ズヴァール城内郭", mob = "Za'Dha's Biographer" },
            { zone = "ウルガラン山脈", mob = "Yagudo Sentinel" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "戦績BF ひとまねことり\n戦績BF ローリングバグズ\n戦績BF タラッカの笛吹き",
        acquisition = "",
    },
    [841] = {
        name = "ディストラ",
        description = "対象の物理回避率をダウン。",
        shop = "マウラ G-9T1 Tya Padolih",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [842] = {
        name = "ディストラII",
        description = "対象の物理回避率をダウン。",
        shop = "タブナジア地下壕 2F F-9T8 Mazuro-Oozuro\nタブナジア地下壕 2F F-7T3 Nilerouche",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [843] = {
        name = "フラズル",
        description = "対象の魔法回避率をダウン。",
        shop = "マウラ G-9T1 Tya Padolih",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [844] = {
        name = "フラズルII",
        description = "対象の魔法回避率をダウン。",
        shop = "タブナジア地下壕 2F F-9T8 Mazuro-Oozuro\nタブナジア地下壕 2F F-7T3 Nilerouche",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [828] = {
        name = "ファイラ",
        description = "自身の周囲の敵に火属性のダメージ。",
        shop = "西アドゥリン ?-?T? Ledericus",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [829] = {
        name = "ファイラII",
        description = "自身の周囲の敵に火属性のダメージ。",
        shop = "西アドゥリン ?-?T? Ledericus",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [830] = {
        name = "ブリザラ",
        description = "自身の周囲の敵に氷属性のダメージ。",
        shop = "西アドゥリン ?-?T? Ledericus",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [831] = {
        name = "ブリザラII",
        description = "自身の周囲の敵に氷属性のダメージ。",
        shop = "西アドゥリン ?-?T? Ledericus",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [832] = {
        name = "エアロラ",
        description = "自身の周囲の敵に風属性のダメージ。",
        shop = "西アドゥリン ?-?T? Ledericus",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [833] = {
        name = "エアロラII",
        description = "自身の周囲の敵に風属性のダメージ。",
        shop = "西アドゥリン ?-?T? Ledericus",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [834] = {
        name = "ストンラ",
        description = "自身の周囲の敵に土属性のダメージ。",
        shop = "西アドゥリン ?-?T? Ledericus",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [835] = {
        name = "ストンラII",
        description = "自身の周囲の敵に土属性のダメージ。",
        shop = "西アドゥリン ?-?T? Ledericus",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [836] = {
        name = "サンダラ",
        description = "自身の周囲の敵に雷属性のダメージ。",
        shop = "西アドゥリン ?-?T? Ledericus",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [837] = {
        name = "サンダラII",
        description = "自身の周囲の敵に雷属性のダメージ。",
        shop = "西アドゥリン ?-?T? Ledericus",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [838] = {
        name = "ウォタラ",
        description = "自身の周囲の敵に水属性のダメージ。",
        shop = "西アドゥリン ?-?T? Ledericus",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [839] = {
        name = "ウォタラII",
        description = "自身の周囲の敵に水属性のダメージ。",
        shop = "西アドゥリン ?-?T? Ledericus",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [320] = {
        name = "火遁の術:壱",
        description = "火属性ダメージ+水耐性ダウンの効果。",
        shop = "バストゥーク港 F-6T5 Jabbar\nジュノ下層 J-8T7 Amalasanda\nノーグ I-8T1 Chiyo",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [321] = {
        name = "火遁の術:弐",
        description = "火属性ダメージ+水耐性ダウンの効果。",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Ziryu" },
            { zone = "ズヴァール城外郭", mob = "Zurko-Bazurko" },
            { zone = "ズヴァール城内郭", mob = "Yowie" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [322] = {
        name = "火遁の術:参",
        description = "火属性ダメージ+水耐性ダウンの効果。",
        shop = "ノーグ H-9T4 Solby-Maholby",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [323] = {
        name = "氷遁の術:壱",
        description = "氷属性ダメージ+火耐性ダウンの効果。",
        shop = "ジュノ下層 J-8T7 Amalasanda\nノーグ I-8T1 Chiyo",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [324] = {
        name = "氷遁の術:弐",
        description = "氷属性ダメージ+火耐性ダウンの効果。",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Ziryu" },
            { zone = "ズヴァール城外郭", mob = "Zurko-Bazurko" },
            { zone = "ズヴァール城内郭", mob = "Yowie" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [325] = {
        name = "氷遁の術:参",
        description = "氷属性ダメージ+火耐性ダウンの効果。",
        shop = "ノーグ H-9T4 Solby-Maholby",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [326] = {
        name = "風遁の術:壱",
        description = "風属性ダメージ+氷耐性ダウンの効果。",
        shop = "バストゥーク港 F-6T5 Jabbar\nジュノ下層 J-8T7 Amalasanda\nノーグ I-8T1 Chiyo",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [327] = {
        name = "風遁の術:弐",
        description = "風属性ダメージ+氷耐性ダウンの効果。",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Ziryu" },
            { zone = "ズヴァール城外郭", mob = "Zurko-Bazurko" },
            { zone = "ズヴァール城内郭", mob = "Yowie" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [328] = {
        name = "風遁の術:参",
        description = "風属性ダメージ+氷耐性ダウンの効果。",
        shop = "ノーグ H-9T4 Solby-Maholby",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [329] = {
        name = "土遁の術:壱",
        description = "土属性ダメージ+風耐性ダウンの効果。",
        shop = "バストゥーク港 F-6T5 Jabbar\nジュノ下層 J-8T7 Amalasanda\nノーグ I-8T1 Chiyo",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [330] = {
        name = "土遁の術:弐",
        description = "土属性ダメージ+風耐性ダウンの効果。",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Ziryu" },
            { zone = "ズヴァール城外郭", mob = "Zurko-Bazurko" },
            { zone = "ズヴァール城内郭", mob = "Yowie" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [331] = {
        name = "土遁の術:参",
        description = "土属性ダメージ+風耐性ダウンの効果。",
        shop = "ノーグ H-9T4 Solby-Maholby",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [332] = {
        name = "雷遁の術:壱",
        description = "雷属性ダメージ+土耐性ダウンの効果。",
        shop = "ジュノ下層 J-8T7 Amalasanda\nノーグ I-8T1 Chiyo",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [333] = {
        name = "雷遁の術:弐",
        description = "雷属性ダメージ+土耐性ダウンの効果。",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Ziryu" },
            { zone = "ズヴァール城外郭", mob = "Zurko-Bazurko" },
            { zone = "ズヴァール城内郭", mob = "Yowie" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [334] = {
        name = "雷遁の術:参",
        description = "雷属性ダメージ+土耐性ダウンの効果。",
        shop = "ノーグ H-9T4 Solby-Maholby",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [335] = {
        name = "水遁の術:壱",
        description = "水属性ダメージ+雷耐性ダウンの効果。",
        shop = "バストゥーク港 F-6T5 Jabbar\nジュノ下層 J-8T7 Amalasanda\nノーグ I-8T1 Chiyo",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [336] = {
        name = "水遁の術:弐",
        description = "水属性ダメージ+雷耐性ダウンの効果。",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Ziryu" },
            { zone = "ズヴァール城外郭", mob = "Zurko-Bazurko" },
            { zone = "ズヴァール城内郭", mob = "Yowie" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [337] = {
        name = "水遁の術:参",
        description = "水属性ダメージ+雷耐性ダウンの効果。",
        shop = "ノーグ H-9T4 Solby-Maholby",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [338] = {
        name = "空蝉の術:壱",
        description = "空蝉の効果。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "辺境 サハギンの宝物庫",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [339] = {
        name = "空蝉の術:弐",
        description = "空蝉の効果。",
        shop = "",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF スカイオーブ 断食芋虫\n印章BF スカイオーブ 天鳴地響\n印章BF スターオーブ 上様御一行\n印章BF スターオーブ 居候妖精\n印章BF スカイオーブ 石竜の双璧\n印章BF スカイオーブ 山岳甲兵\n印章BF スカイオーブ 降下猟兵\n印章BF スターオーブ 地獄の尻尾\n印章BF スターオーブ 光る眼\n印章BF スカイオーブ 剣闘士の得物\n印章BF スターオーブ 人形工房\n印章BF スカイオーブ 凶狗四天王\n印章BF スターオーブ 生ける屍\n印章BF スカイオーブ 飛行小隊\n印章BF スカイオーブ 森の乙女\n印章BF スターオーブ 噂のスライム\n印章BF スターオーブ 地竜大王",
        acquisition = "",
    },
    [341] = {
        name = "呪縛の術:壱",
        description = "麻痺の効果。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "辺境 濡れた巻物の秘密",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [344] = {
        name = "捕縄の術:壱",
        description = "スロウの効果。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "辺境 大自然の極上酒",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [345] = {
        name = "捕縄の術:弐",
        description = "スロウの効果。",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Yoo Mihi the Haze" },
            { zone = "ソ・ジヤ", mob = "Zoraal Ja's Pkuucha" },
            { zone = "ヨアトル大森林", mob = "Yoo Mihi the Haze" },
            { zone = "ウガレピ寺院", mob = "Young Quadav" },
            { zone = "ウガレピ寺院", mob = "Zirnitra" },
            { zone = "ウガレピ寺院", mob = "Zoraal Ja's Pkuucha" },
            { zone = "怨念洞", mob = "Yagudo Scribe" },
            { zone = "怨念洞", mob = "Yara Ma Yha Who" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [347] = {
        name = "暗闇の術:壱",
        description = "暗闇の効果。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "辺境 輝けるサブリガ",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [348] = {
        name = "暗闇の術:弐",
        description = "暗闇の効果。",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Yoo Mihi the Haze" },
            { zone = "ソ・ジヤ", mob = "Zoraal Ja's Pkuucha" },
            { zone = "ウガレピ寺院", mob = "Young Quadav" },
            { zone = "ウガレピ寺院", mob = "Zirnitra" },
            { zone = "ウガレピ寺院", mob = "Zoraal Ja's Pkuucha" },
            { zone = "怨念洞", mob = "Yagudo Scribe" },
            { zone = "怨念洞", mob = "Yara Ma Yha Who" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [350] = {
        name = "毒盛の術:壱",
        description = "毒の効果。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "辺境 輝けるレギンス",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [353] = {
        name = "遁甲の術:壱",
        description = "インビジの効果。",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [354] = {
        name = "遁甲の術:弐",
        description = "インビジの効果。",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Yoo Mihi the Haze" },
            { zone = "ソ・ジヤ", mob = "Zoraal Ja's Pkuucha" },
            { zone = "ヨアトル大森林", mob = "Yoo Mihi the Haze" },
            { zone = "ウガレピ寺院", mob = "Young Quadav" },
            { zone = "ウガレピ寺院", mob = "Zirnitra" },
            { zone = "ウガレピ寺院", mob = "Zoraal Ja's Pkuucha" },
            { zone = "怨念洞", mob = "Yagudo Scribe" },
            { zone = "怨念洞", mob = "Yara Ma Yha Who" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [318] = {
        name = "物見の術:壱",
        description = "スニークの効果。",
        shop = "ジュノ下層 J-8T7 Amalasanda\nカザム I-8T7 Toji Mumosulah",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [319] = {
        name = "哀車の術:壱",
        description = "攻撃力ダウンの効果。",
        shop = "ノーグ H-9T4 Solby-Maholby",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [507] = {
        name = "妙手の術:壱",
        description = "モクシャアップの効果。",
        shop = "ノーグ H-9T4 Solby-Maholby",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [508] = {
        name = "幽林の術:壱",
        description = "インヒビットTPの効果。",
        shop = "ノーグ H-9T4 Solby-Maholby",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [509] = {
        name = "活火の術:壱",
        description = "ストアTPアップの効果。",
        shop = "ノーグ H-9T4 Solby-Maholby",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [505] = {
        name = "月下の術:壱",
        description = "敵対心アップの効果。",
        shop = "ノーグ H-9T4 Solby-Maholby",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [506] = {
        name = "夜陰の術:壱",
        description = "パクスの効果",
        shop = "ノーグ H-9T4 Solby-Maholby",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [368] = {
        name = "魔物のレクイエム",
        description = "敵に音波ダメージ",
        shop = "バストゥーク商業区 K-10T6 Hortense",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [369] = {
        name = "魔物のレクイエムII",
        description = "敵に音波ダメージ",
        shop = "バストゥーク商業区 K-10T6 Hortense",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [370] = {
        name = "魔物のレクイエムIII",
        description = "敵に音波ダメージ",
        shop = "バストゥーク商業区 K-10T6 Hortense",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [371] = {
        name = "魔物のレクイエムIV",
        description = "敵に音波ダメージ",
        shop = "バストゥーク商業区 K-10T6 Hortense",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [372] = {
        name = "魔物のレクイエムV",
        description = "敵に音波ダメージ",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Zeid" },
            { zone = "ズヴァール城内郭", mob = "Zhu Que" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [373] = {
        name = "魔物のレクイエムVI",
        description = "敵に音波ダメージ",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Yara Ma Yha Who" },
            { zone = "オズトロヤ城", mob = "Zvahl Fortalice" },
            { zone = "海蛇の岩窟", mob = "Zagh" },
            { zone = "海蛇の岩窟", mob = "Zo'Bha Helmkeeper" },
            { zone = "海蛇の岩窟", mob = "Zolku-Azolku" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [374] = {
        name = "魔物のレクイエムVII",
        description = "敵に音波ダメージ",
        shop = "バストゥーク商業区 K-10T6 Hortense",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [376] = {
        name = "魔物達のララバイ",
        description = "範囲内の敵を睡眠状態にする",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Yagudo Scribe" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF クラウディオーブ 恐怖の狙撃手",
        acquisition = "",
    },
    [378] = {
        name = "戦士達のピーアン",
        description = "範囲内のパーティメンバーのHPを徐々に回復",
        shop = "バストゥーク商業区 K-10T6 Hortense",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [379] = {
        name = "戦士達のピーアンII",
        description = "範囲内のパーティメンバーのHPを徐々に回復",
        shop = "バストゥーク商業区 K-10T6 Hortense",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [380] = {
        name = "戦士達のピーアンIII",
        description = "範囲内のパーティメンバーのHPを徐々に回復",
        shop = "バストゥーク商業区 K-10T6 Hortense\nカザム I-8T7 Toji Mumosulah",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [381] = {
        name = "戦士達のピーアンIV",
        description = "範囲内のパーティメンバーのHPを徐々に回復",
        shop = "バストゥーク商業区 K-10T6 Hortense",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [382] = {
        name = "戦士達のピーアンV",
        description = "範囲内のパーティメンバーのHPを徐々に回復",
        shop = "",
        quest = "",
        drop = {
            { zone = "海蛇の岩窟", mob = "Zo'Bha Helmkeeper" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "ENM 風と共に\nENM 巨人の貝塚",
        acquisition = "",
    },
    [383] = {
        name = "戦士達のピーアンVI",
        description = "範囲内のパーティメンバーのHPを徐々に回復",
        shop = "バストゥーク商業区 K-10T6 Hortense",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [386] = {
        name = "魔道士のバラード",
        description = "範囲内のパーティメンバーのMPを徐々に回復",
        shop = "ジュノ下層 J-8T7 Amalasanda",
        quest = "サンドリア パーティの心得～中級編",
        drop = {
            { zone = "ギデアス", mob = "Zeid" },
            { zone = "ギデアス", mob = "Ziz" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [388] = {
        name = "魔道士のバラードIII",
        description = "範囲内のパーティメンバーのMPを徐々に回復",
        shop = "南サンドリア（コンクエ1位） H-9T9 Valeriano\nバストゥーク港（コンクエ1位） K-8T5 Valeriano\nウィンダス森の区（コンクエ1位） H-10T3 Valeriano",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [389] = {
        name = "重装騎兵のミンネ",
        description = "範囲内のパーティメンバーの防御力をアップ",
        shop = "南サンドリア I-11T6 Ferdoulemiont",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [390] = {
        name = "重装騎兵のミンネII",
        description = "範囲内のパーティメンバーの防御力をアップ",
        shop = "南サンドリア I-11T6 Ferdoulemiont",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [391] = {
        name = "重装騎兵のミンネIII",
        description = "範囲内のパーティメンバーの防御力をアップ",
        shop = "南サンドリア I-11T6 Ferdoulemiont",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [392] = {
        name = "重装騎兵のミンネIV",
        description = "範囲内のパーティメンバーの防御力をアップ",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Yara Ma Yha Who" },
            { zone = "オズトロヤ城", mob = "Zvahl Fortalice" },
            { zone = "海蛇の岩窟", mob = "Zagh" },
            { zone = "海蛇の岩窟", mob = "Zo'Bha Helmkeeper" },
            { zone = "海蛇の岩窟", mob = "Zolku-Azolku" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [393] = {
        name = "重装騎兵のミンネV",
        description = "範囲内のパーティメンバーの防御力をアップ",
        shop = "南サンドリア I-11T6 Ferdoulemiont",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [394] = {
        name = "猛者のメヌエット",
        description = "範囲内のパーティメンバーの攻撃力をアップ",
        shop = "バストゥーク商業区 K-10T6 Hortense",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [395] = {
        name = "猛者のメヌエットII",
        description = "範囲内のパーティメンバーの攻撃力をアップ",
        shop = "バストゥーク商業区 K-10T6 Hortense",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [396] = {
        name = "猛者のメヌエットIII",
        description = "範囲内のパーティメンバーの攻撃力をアップ",
        shop = "バストゥーク商業区 K-10T6 Hortense",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [397] = {
        name = "猛者のメヌエットIV",
        description = "範囲内のパーティメンバーの攻撃力をアップ",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Yara Ma Yha Who" },
            { zone = "オズトロヤ城", mob = "Zvahl Fortalice" },
            { zone = "海蛇の岩窟", mob = "Zagh" },
            { zone = "海蛇の岩窟", mob = "Zo'Bha Helmkeeper" },
            { zone = "海蛇の岩窟", mob = "Zolku-Azolku" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF ムーンオーブ ドーフェ兄弟\n印章BF ムーンオーブ 攻城剛力組",
        acquisition = "",
    },
    [398] = {
        name = "猛者のメヌエットV",
        description = "範囲内のパーティメンバーの攻撃力をアップ",
        shop = "バストゥーク商業区 K-10T6 Hortense",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [399] = {
        name = "剣闘士のマドリガル",
        description = "範囲内のパーティメンバーの近接攻撃の命中率をアップ",
        shop = "北サンドリア F-8T1 Boncort",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [400] = {
        name = "剣豪のマドリガル",
        description = "範囲内のパーティメンバーの近接攻撃の命中率をアップ",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Ze'Vho Fallsplitter" },
            { zone = "ズヴァール城外郭", mob = "Yearner" },
            { zone = "ズヴァール城内郭", mob = "Yara Ma Yha Who" },
            { zone = "西サルタバルタ〔Ｓ〕", mob = "Zurko-Bazurko" },
            { zone = "カルゴナルゴ城砦〔Ｓ〕", mob = "Ziz" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [401] = {
        name = "狩人のプレリュード",
        description = "範囲内のパーティメンバーの遠隔攻撃の命中率をアップ",
        shop = "ウィンダス森の区 H-13T6 Mono Nchaa",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [402] = {
        name = "弓師のプレリュード",
        description = "範囲内のパーティメンバーの遠隔攻撃の命中率をアップ",
        shop = "",
        quest = "",
        drop = {
            { zone = "ダボイ", mob = "Yoo Mihi the Haze" },
            { zone = "修道窟", mob = "Zircon Quadav" },
            { zone = "デルクフの塔上層", mob = "Zombie" },
            { zone = "流砂洞", mob = "Zurko-Bazurko" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [403] = {
        name = "闘羊士のマンボ",
        description = "範囲内のパーティメンバーの回避率をアップ",
        shop = "セルビナ G-10T7 Dohdjuma",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [404] = {
        name = "闘龍士のマンボ",
        description = "範囲内のパーティメンバーの回避率をアップ",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Ze'Vho Fallsplitter" },
            { zone = "ズヴァール城外郭", mob = "Yearner" },
            { zone = "ズヴァール城内郭", mob = "Yara Ma Yha Who" },
            { zone = "西サルタバルタ〔Ｓ〕", mob = "Zurko-Bazurko" },
            { zone = "カルゴナルゴ城砦〔Ｓ〕", mob = "Ziz" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [405] = {
        name = "鶏のオーバード",
        description = "範囲内のパーティメンバーの睡眠耐性をアップ",
        shop = "南サンドリア（コンクエ1位） H-9T9 Valeriano\nバストゥーク港（コンクエ1位） K-8T5 Valeriano\nウィンダス森の区（コンクエ1位） H-10T3 Valeriano",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [406] = {
        name = "薬草のパストラル",
        description = "範囲内のパーティメンバーの毒耐性をアップ",
        shop = "ウィンダス森の区 I-8T9 Wije Tiren",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [408] = {
        name = "光明のファンタジア",
        description = "範囲内のパーティメンバーの暗闇耐性をアップ",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Yara Ma Yha Who" },
            { zone = "オズトロヤ城", mob = "Zvahl Fortalice" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [409] = {
        name = "小話のオペレッタ",
        description = "範囲内のパーティメンバーの静寂耐性をアップ",
        shop = "南サンドリア（コンクエ1位） H-9T9 Valeriano\nバストゥーク港（コンクエ1位） K-8T5 Valeriano\nウィンダス森の区（コンクエ1位） H-10T3 Valeriano",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [410] = {
        name = "腹話のオペレッタ",
        description = "範囲内のパーティメンバーの静寂耐性をアップ",
        shop = "南サンドリア（コンクエ1位） H-9T9 Valeriano\nバストゥーク港（コンクエ1位） K-8T5 Valeriano\nウィンダス森の区（コンクエ1位） H-10T3 Valeriano",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [412] = {
        name = "黄金のカプリチオ",
        description = "範囲内のパーティメンバーの石化耐性をアップ",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Zeid" },
            { zone = "ズヴァール城内郭", mob = "Zhu Que" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [414] = {
        name = "破邪のロンド",
        description = "範囲内のパーティメンバーの呪い耐性をアップ",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Yara Ma Yha Who" },
            { zone = "オズトロヤ城", mob = "Zvahl Fortalice" },
            { zone = "海蛇の岩窟", mob = "Zo'Bha Helmkeeper" },
            { zone = "メリファト山地〔Ｓ〕", mob = "Yagudo Votary" },
            { zone = "ソロムグ原野〔Ｓ〕", mob = "Zeid" },
            { zone = "オズトロヤ城〔Ｓ〕", mob = "Zhayolm Apkallu" },
            { zone = "オズトロヤ城〔Ｓ〕", mob = "Zu" },
            { zone = "ガルレージュ要塞〔Ｓ〕", mob = "Zircon Quadav" },
            { zone = "ズヴァール城外郭〔Ｓ〕", mob = "Zurko-Bazurko" },
            { zone = "ズヴァール城内郭〔Ｓ〕", mob = "Ziryu" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [415] = {
        name = "ゴブリンのガボット",
        description = "範囲内のパーティメンバーのバインド耐性をアップ",
        shop = "ジュノ下層 H-9T2 Creepstix",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [419] = {
        name = "無敵の進撃マーチ",
        description = "範囲内のパーティメンバーの攻撃間隔を短縮",
        shop = "南サンドリア（コンクエ1位） H-9T9 Valeriano\nバストゥーク港（コンクエ1位） K-8T5 Valeriano\nウィンダス森の区（コンクエ1位） H-10T3 Valeriano",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [420] = {
        name = "栄光の凱旋マーチ",
        description = "範囲内のパーティメンバーの攻撃間隔を短縮",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Yara Ma Yha Who" },
            { zone = "オズトロヤ城", mob = "Zvahl Fortalice" },
            { zone = "海蛇の岩窟", mob = "Zolku-Azolku" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [421] = {
        name = "戦場のエレジー",
        description = "敵の攻撃間隔を長くする",
        shop = "南サンドリア K-7T4 Thadiene",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [422] = {
        name = "修羅のエレジー",
        description = "敵の攻撃間隔を長くする",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Zeid" },
            { zone = "ズヴァール城内郭", mob = "Zhu Que" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "戦績BF ひとまねことり\n戦績BF ローリングバグズ\n戦績BF タラッカの笛吹き\n印章BF ムーンオーブ 蒼の血族",
        acquisition = "",
    },
    [424] = {
        name = "剛力のエチュード",
        description = "範囲内のパーティメンバーのSTRアップ",
        shop = "ウィンダス森の区 H-13T6 Manyny\nジュノ港 H-8T5 Leyla",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [425] = {
        name = "器用のエチュード",
        description = "範囲内のパーティメンバーのDEXアップ",
        shop = "ウィンダス森の区 H-13T6 Manyny\nジュノ港 H-8T5 Leyla",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [426] = {
        name = "元気のエチュード",
        description = "範囲内のパーティメンバーのVITアップ",
        shop = "ウィンダス森の区 H-13T6 Manyny\nジュノ港 H-8T5 Leyla",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [427] = {
        name = "機敏のエチュード",
        description = "範囲内のパーティメンバーのAGIアップ",
        shop = "ウィンダス森の区 H-13T6 Manyny\nジュノ港 H-8T5 Leyla",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [428] = {
        name = "知恵のエチュード",
        description = "範囲内のパーティメンバーのINTアップ",
        shop = "ウィンダス森の区 H-13T6 Manyny\nジュノ港 H-8T5 Leyla",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [429] = {
        name = "精神のエチュード",
        description = "範囲内のパーティメンバーのMNDアップ",
        shop = "ウィンダス森の区 H-13T6 Manyny\nジュノ港 H-8T5 Leyla",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [430] = {
        name = "魅了のエチュード",
        description = "範囲内のパーティメンバーのCHRアップ",
        shop = "ウィンダス森の区 H-13T6 Manyny\nジュノ港 H-8T5 Leyla",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [431] = {
        name = "怪力のエチュード",
        description = "範囲内のパーティメンバーのSTRアップ",
        shop = "バストゥーク商業区 K-10T6 Harmodios",
        quest = "",
        drop = {
            { zone = "海蛇の岩窟", mob = "Zagh" },
            { zone = "海蛇の岩窟", mob = "Zo'Bha Helmkeeper" },
            { zone = "海蛇の岩窟", mob = "Zolku-Azolku" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [432] = {
        name = "妙技のエチュード",
        description = "範囲内のパーティメンバーのDEXアップ",
        shop = "バストゥーク商業区 K-10T6 Harmodios",
        quest = "",
        drop = {
            { zone = "海蛇の岩窟", mob = "Zagh" },
            { zone = "海蛇の岩窟", mob = "Zo'Bha Helmkeeper" },
            { zone = "海蛇の岩窟", mob = "Zolku-Azolku" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [433] = {
        name = "活力のエチュード",
        description = "範囲内のパーティメンバーのVITアップ",
        shop = "バストゥーク商業区 K-10T6 Harmodios",
        quest = "",
        drop = {
            { zone = "海蛇の岩窟", mob = "Zagh" },
            { zone = "海蛇の岩窟", mob = "Zo'Bha Helmkeeper" },
            { zone = "海蛇の岩窟", mob = "Zolku-Azolku" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [434] = {
        name = "俊敏のエチュード",
        description = "範囲内のパーティメンバーのAGIアップ",
        shop = "バストゥーク商業区 K-10T6 Harmodios",
        quest = "",
        drop = {
            { zone = "海蛇の岩窟", mob = "Zagh" },
            { zone = "海蛇の岩窟", mob = "Zo'Bha Helmkeeper" },
            { zone = "海蛇の岩窟", mob = "Zolku-Azolku" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [435] = {
        name = "英知のエチュード",
        description = "範囲内のパーティメンバーのINTアップ",
        shop = "バストゥーク商業区 K-10T6 Harmodios",
        quest = "",
        drop = {
            { zone = "海蛇の岩窟", mob = "Zagh" },
            { zone = "海蛇の岩窟", mob = "Zo'Bha Helmkeeper" },
            { zone = "海蛇の岩窟", mob = "Zolku-Azolku" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [436] = {
        name = "理力のエチュード",
        description = "範囲内のパーティメンバーのMNDアップ",
        shop = "バストゥーク商業区 K-10T6 Harmodios",
        quest = "",
        drop = {
            { zone = "海蛇の岩窟", mob = "Zagh" },
            { zone = "海蛇の岩窟", mob = "Zo'Bha Helmkeeper" },
            { zone = "海蛇の岩窟", mob = "Zolku-Azolku" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [437] = {
        name = "魅惑のエチュード",
        description = "範囲内のパーティメンバーのCHRアップ",
        shop = "バストゥーク商業区 K-10T6 Harmodios",
        quest = "",
        drop = {
            { zone = "海蛇の岩窟", mob = "Zagh" },
            { zone = "海蛇の岩窟", mob = "Zo'Bha Helmkeeper" },
            { zone = "海蛇の岩窟", mob = "Zolku-Azolku" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [438] = {
        name = "耐火カロル第一楽章",
        description = "範囲内のパーティメンバーの火属性に対する防御力アップ",
        shop = "ジュノ下層 I-8T6 Yoskolo",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [439] = {
        name = "耐寒カロル第一楽章",
        description = "範囲内のパーティメンバーの氷属性に対する防御力アップ",
        shop = "ジュノ下層 I-8T6 Yoskolo",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [440] = {
        name = "耐風カロル第一楽章",
        description = "範囲内のパーティメンバーの風属性に対する防御力アップ",
        shop = "ジュノ下層 I-8T6 Yoskolo",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [441] = {
        name = "耐震カロル第一楽章",
        description = "範囲内のパーティメンバーの土属性に対する防御力アップ",
        shop = "ジュノ下層 I-8T6 Yoskolo",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [442] = {
        name = "耐電カロル第一楽章",
        description = "範囲内のパーティメンバーの雷属性に対する防御力アップ",
        shop = "ジュノ下層 I-8T6 Yoskolo",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [443] = {
        name = "耐波カロル第一楽章",
        description = "範囲内のパーティメンバーの水属性に対する防御力アップ",
        shop = "ジュノ下層 I-8T6 Yoskolo",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [444] = {
        name = "耐光カロル第一楽章",
        description = "範囲内のパーティメンバーの光属性に対する防御力アップ",
        shop = "ジュノ下層 I-8T6 Yoskolo",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [445] = {
        name = "耐闇カロル第一楽章",
        description = "範囲内のパーティメンバーの闇属性に対する防御力アップ",
        shop = "ジュノ下層 I-8T6 Yoskolo",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [446] = {
        name = "耐火カロル第二楽章",
        description = "範囲内のパーティメンバーの火属性に対する防御力をアップし、時々火属性ダメージを無効化する",
        shop = "南サンドリア（コンクエ1位） H-9T9 Valeriano\nバストゥーク港（コンクエ1位） K-8T5 Valeriano\nウィンダス森の区（コンクエ1位） H-10T3 Valeriano",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [448] = {
        name = "耐風カロル第二楽章",
        description = "範囲内のパーティメンバーの風属性に対する防御力をアップし、時々風属性ダメージを無効化する",
        shop = "南サンドリア（コンクエ1位） H-9T9 Valeriano\nバストゥーク港（コンクエ1位） K-8T5 Valeriano\nウィンダス森の区（コンクエ1位） H-10T3 Valeriano",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [449] = {
        name = "耐震カロル第二楽章",
        description = "範囲内のパーティメンバーの土属性に対する防御力をアップし、時々土属性ダメージを無効化する",
        shop = "南サンドリア（コンクエ1位） H-9T9 Valeriano\nバストゥーク港（コンクエ1位） K-8T5 Valeriano\nウィンダス森の区（コンクエ1位） H-10T3 Valeriano",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [451] = {
        name = "耐波カロル第二楽章",
        description = "範囲内のパーティメンバーの水属性に対する防御力をアップし、時々水属性ダメージを無効化する",
        shop = "南サンドリア（コンクエ1位） H-9T9 Valeriano\nバストゥーク港（コンクエ1位） K-8T5 Valeriano\nウィンダス森の区（コンクエ1位） H-10T3 Valeriano",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [454] = {
        name = "炎のスレノディ",
        description = "敵の火属性に対する防御力ダウン",
        shop = "ウィンダス水の区 北 H-9T8 Ensasa",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [455] = {
        name = "氷のスレノディ",
        description = "敵の氷属性に対する防御力ダウン",
        shop = "バストゥーク商業区 E-11T6 Mjoll",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [456] = {
        name = "風のスレノディ",
        description = "敵の風属性に対する防御力ダウン",
        shop = "ジュノ上層 H-6T4 Champalpieu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [457] = {
        name = "土のスレノディ",
        description = "敵の土属性に対する防御力ダウン",
        shop = "ウィンダス水の区 北 H-9T8 Ensasa",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [458] = {
        name = "雷のスレノディ",
        description = "敵の雷属性に対する防御力ダウン",
        shop = "南サンドリア F-8T3 Lusiane",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [459] = {
        name = "水のスレノディ",
        description = "敵の水属性に対する防御力ダウン",
        shop = "ジュノ上層 H-6T4 Champalpieu",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [460] = {
        name = "光のスレノディ",
        description = "敵の光属性に対する防御力ダウン",
        shop = "南サンドリア F-8T3 Lusiane",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [461] = {
        name = "闇のスレノディ",
        description = "敵の闇属性に対する防御力ダウン",
        shop = "バストゥーク商業区 E-11T6 Mjoll",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [462] = {
        name = "魔法のフィナーレ",
        description = "敵の魔法効果を1つ消し去る",
        shop = "",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Yagudo Scribe" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "印章BF スカイオーブ 断食芋虫\n印章BF スカイオーブ 天鳴地響\n印章BF スカイオーブ 石竜の双璧\n印章BF スカイオーブ 山岳甲兵\n印章BF スカイオーブ 降下猟兵\n印章BF スカイオーブ 剣闘士の得物\n印章BF スカイオーブ 凶狗四天王\n印章BF スカイオーブ 飛行小隊\n印章BF スカイオーブ 森の乙女",
        acquisition = "",
    },
    [463] = {
        name = "魔物のララバイ",
        description = "敵を睡眠状態にする",
        shop = "",
        quest = "",
        drop = {
            { zone = "ギデアス", mob = "Ziz" },
            { zone = "タロンギ大峡谷", mob = "Yearner" },
            { zone = "メリファト山地", mob = "Yearner" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [464] = {
        name = "女神のヒムヌス",
        description = "範囲内のパーティメンバーにリレイズの効果",
        shop = "南サンドリア（コンクエ1位） H-9T9 Valeriano\nバストゥーク港（コンクエ1位） K-8T5 Valeriano\nウィンダス森の区（コンクエ1位） H-10T3 Valeriano",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Yara Ma Yha Who" },
            { zone = "オズトロヤ城", mob = "Zvahl Fortalice" },
            { zone = "海蛇の岩窟", mob = "Zo'Bha Helmkeeper" },
            { zone = "メリファト山地〔Ｓ〕", mob = "Yagudo Votary" },
            { zone = "ソロムグ原野〔Ｓ〕", mob = "Zeid" },
            { zone = "オズトロヤ城〔Ｓ〕", mob = "Zhayolm Apkallu" },
            { zone = "オズトロヤ城〔Ｓ〕", mob = "Zu" },
            { zone = "ガルレージュ要塞〔Ｓ〕", mob = "Zircon Quadav" },
            { zone = "ズヴァール城外郭〔Ｓ〕", mob = "Zurko-Bazurko" },
            { zone = "ズヴァール城内郭〔Ｓ〕", mob = "Ziryu" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [465] = {
        name = "チョコボのマズルカ",
        description = "範囲内のパーティメンバーの移動速度をアップ",
        shop = "南サンドリア I-11T6 Ferdoulemiont\nバストゥーク鉱山区 J-9T4 Neigepance\nウィンダス森の区 K-12T3 Quesse\nラバオ G-6T1 Generoit\nカザム F-9T5 Mamerie",
        quest = "",
        drop = {
            { zone = "オズトロヤ城", mob = "Yara Ma Yha Who" },
            { zone = "オズトロヤ城", mob = "Zvahl Fortalice" },
            { zone = "海蛇の岩窟", mob = "Zagh" },
            { zone = "海蛇の岩窟", mob = "Zo'Bha Helmkeeper" },
            { zone = "海蛇の岩窟", mob = "Zolku-Azolku" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [466] = {
        name = "乙女のヴィルレー",
        description = "敵を魅了する",
        shop = "",
        quest = "",
        drop = {
            { zone = "海蛇の岩窟", mob = "Zolku-Azolku" },
        },
        steal = {},
        chest = "",
        mining = "",
        content = "戦績BF ひとまねことり\n戦績BF ローリングバグズ\n戦績BF タラッカの笛吹き",
        acquisition = "",
    },
    [467] = {
        name = "ラプトルのマズルカ",
        description = "範囲内のパーティメンバーの移動速度をアップ",
        shop = "アトルガン白門 H-11T2 Khaf Jhifanm",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [468] = {
        name = "魔物のシルベント",
        description = "範囲内のパーティメンバーの敵対心を下がりにくくする",
        shop = "バストゥーク商業区 K-10T6 Harmodios",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [469] = {
        name = "冒険者のダージュ",
        description = "範囲内のパーティメンバーの敵対心をダウン",
        shop = "バストゥーク商業区 K-10T6 Harmodios",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [470] = {
        name = "警戒のスケルツォ",
        description = "範囲内のパーティメンバーの大ダメージを軽減する",
        shop = "ジュノ下層 I-8T6 Yoskolo",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [471] = {
        name = "魔物達のララバイII",
        description = "範囲内の敵を睡眠状態にする",
        shop = "カザム I-8T7 Toji Mumosulah",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [474] = {
        name = "ケアルラII",
        description = "範囲内のパーティメンバーのHPを回復。ハートオブミゼリ:回復量にボーナスを得る。",
        shop = "ラバオ F-7T8 Brave Ox",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [477] = {
        name = "リジェネIV",
        description = "標的のパーティメンバーのHPを徐々に回復する。",
        shop = "ジュノ港 H-8T5 Gekko",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [486] = {
        name = "ゲインスト",
        description = "自身のSTRアップ。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [487] = {
        name = "ゲインデック",
        description = "自身のDEXアップ。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [488] = {
        name = "ゲインバイト",
        description = "自身のVITアップ。",
        shop = "ジュノ下層 H-9T2 Creepstix\nジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [489] = {
        name = "ゲインアジル",
        description = "自身のAGIアップ。",
        shop = "ジュノ下層 H-9T2 Creepstix\nジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [490] = {
        name = "ゲインイン",
        description = "自身のINTアップ。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [491] = {
        name = "ゲインマイン",
        description = "自身のMNDアップ。",
        shop = "ジュノ下層 H-9T2 Creepstix\nジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [492] = {
        name = "ゲインカリス",
        description = "自身のCHRアップ。",
        shop = "ジュノ下層 H-9T2 Creepstix\nジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [479] = {
        name = "アディスト",
        description = "範囲内のパーティメンバーのSTRアップ。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [480] = {
        name = "アディデック",
        description = "範囲内のパーティメンバーのDEXアップ。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [481] = {
        name = "アディバイト",
        description = "範囲内のパーティメンバーのVITアップ。",
        shop = "ジュノ下層 H-9T2 Creepstix\nジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [482] = {
        name = "アディアジル",
        description = "範囲内のパーティメンバーのAGIアップ。",
        shop = "ジュノ下層 H-9T2 Creepstix\nジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [483] = {
        name = "アディイン",
        description = "範囲内のパーティメンバーのINTアップ。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [484] = {
        name = "アディマイン",
        description = "範囲内のパーティメンバーのMNDアップ。",
        shop = "ジュノ下層 H-9T2 Creepstix\nジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [485] = {
        name = "アディカリス",
        description = "範囲内のパーティメンバーのCHRアップ。",
        shop = "ジュノ下層 H-9T2 Creepstix\nジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [840] = {
        name = "フォイル",
        description = "自身の特殊攻撃への回避率をアップ。",
        shop = "西アドゥリン ?-?T? Ledericus",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [476] = {
        name = "クルセード",
        description = "自身の敵対心を上昇しやすくする。",
        shop = "ラバオ F-7T8 Brave Ox",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [845] = {
        name = "スナップ",
        description = "標的の遠隔攻撃の間隔を短くする。",
        shop = "セルビナ H-9T5 Falgima",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [846] = {
        name = "スナップII",
        description = "標的の遠隔攻撃の間隔を短くする。",
        shop = "西アドゥリン ?-?T? Ledericus",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [879] = {
        name = "イナンデーション",
        description = "標的に異なる種別の武器のウェポンスキルを使用すると、連携ダメージアップ。",
        shop = "ジュノ下層 H-9T2 Creepstix\nジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [281] = {
        name = "火門の計",
        description = "敵を火属性のダメージがじわじわ蝕む。天候や曜日に大きく影響される。",
        shop = "エルディーム古墳〔Ｓ〕 J-8T4 Layton",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [279] = {
        name = "水門の計",
        description = "敵を水属性のダメージがじわじわ蝕む。天候や曜日に大きく影響される。",
        shop = "エルディーム古墳〔Ｓ〕 J-8T4 Layton",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [283] = {
        name = "雷門の計",
        description = "敵を雷属性のダメージがじわじわ蝕む。天候や曜日に大きく影響される。",
        shop = "エルディーム古墳〔Ｓ〕 J-8T4 Layton",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [282] = {
        name = "氷門の計",
        description = "敵を氷属性のダメージがじわじわ蝕む。天候や曜日に大きく影響される。",
        shop = "エルディーム古墳〔Ｓ〕 J-8T4 Layton",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [278] = {
        name = "土門の計",
        description = "敵を土属性のダメージがじわじわ蝕む。天候や曜日に大きく影響される。",
        shop = "エルディーム古墳〔Ｓ〕 J-8T4 Layton",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [280] = {
        name = "風門の計",
        description = "敵を風属性のダメージがじわじわ蝕む。天候や曜日に大きく影響される。",
        shop = "エルディーム古墳〔Ｓ〕 J-8T4 Layton",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [285] = {
        name = "光門の計",
        description = "敵を光属性のダメージがじわじわ蝕む。天候や曜日に大きく影響される。",
        shop = "エルディーム古墳〔Ｓ〕 J-8T4 Layton",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [284] = {
        name = "闇門の計",
        description = "敵を闇属性のダメージがじわじわ蝕む。天候や曜日に大きく影響される。",
        shop = "エルディーム古墳〔Ｓ〕 J-8T4 Layton",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [115] = {
        name = "熱波の陣",
        description = "標的のパーティメンバーの周囲の天候を熱波にする。",
        shop = "エルディーム古墳〔Ｓ〕 J-8T4 Layton",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [113] = {
        name = "豪雨の陣",
        description = "標的のパーティメンバーの周囲の天候を雨にする。",
        shop = "エルディーム古墳〔Ｓ〕 J-8T4 Layton",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [117] = {
        name = "疾雷の陣",
        description = "標的のパーティメンバーの周囲の天候を雷にする。",
        shop = "エルディーム古墳〔Ｓ〕 J-8T4 Layton",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [116] = {
        name = "吹雪の陣",
        description = "標的のパーティメンバーの周囲の天候を雪にする。",
        shop = "エルディーム古墳〔Ｓ〕 J-8T4 Layton",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [99] = {
        name = "砂塵の陣",
        description = "標的のパーティメンバーの周囲の天候を砂塵にする。",
        shop = "エルディーム古墳〔Ｓ〕 J-8T4 Layton",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [114] = {
        name = "烈風の陣",
        description = "標的のパーティメンバーの周囲の天候を風にする。",
        shop = "エルディーム古墳〔Ｓ〕 J-8T4 Layton",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [119] = {
        name = "極光の陣",
        description = "標的のパーティメンバーの周囲の天候をオーロラにする。",
        shop = "エルディーム古墳〔Ｓ〕 J-8T4 Layton",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [118] = {
        name = "妖霧の陣",
        description = "標的のパーティメンバーの周囲の天候を妖霧にする。",
        shop = "エルディーム古墳〔Ｓ〕 J-8T4 Layton",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [308] = {
        name = "悪事千里の策",
        description = "標的のパーティメンバーの敵対心を上がりやすくする。",
        shop = "バストゥーク商業区〔Ｓ〕 L-10T5 Silke",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [309] = {
        name = "暗中飛躍の策",
        description = "標的のパーティメンバーの敵対心を上がりにくくする。",
        shop = "バストゥーク商業区〔Ｓ〕 L-10T5 Silke",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [495] = {
        name = "鼓舞激励の策",
        description = "標的のパーティメンバーのTPを徐々に増加する。",
        shop = "バストゥーク商業区〔Ｓ〕 L-10T5 Silke",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [768] = {
        name = "インデリゲイン",
        description = "",
        shop = "西アドゥリン ?-?T? Ishvad",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [769] = {
        name = "インデポイズン",
        description = "自身の周囲の敵は毒で徐々にHPを失う。",
        shop = "西アドゥリン ?-?T? Ishvad",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [770] = {
        name = "インデリフレシュ",
        description = "自身の周囲のパーティメンバーのMPを徐々に回復。",
        shop = "西アドゥリン ?-?T? Ishvad",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [772] = {
        name = "インデスト",
        description = "自身の周囲のパーティメンバーのSTRアップ。",
        shop = "西アドゥリン ?-?T? Eukalline",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [773] = {
        name = "インデデック",
        description = "自身の周囲のパーティメンバーのDEXアップ。",
        shop = "西アドゥリン ?-?T? Ishvad",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [774] = {
        name = "インデバイト",
        description = "自身の周囲のパーティメンバーのVITアップ。",
        shop = "西アドゥリン ?-?T? Ishvad",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [775] = {
        name = "インデアジル",
        description = "自身の周囲のパーティメンバーのAGIアップ。",
        shop = "西アドゥリン ?-?T? Ishvad",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [776] = {
        name = "インデイン",
        description = "自身の周囲のパーティメンバーのINTアップ。",
        shop = "西アドゥリン ?-?T? Ishvad",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [777] = {
        name = "インデマイン",
        description = "自身の周囲のパーティメンバーのMNDアップ。",
        shop = "西アドゥリン ?-?T? Ishvad",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [778] = {
        name = "インデカリス",
        description = "自身の周囲のパーティメンバーのCHRアップ。",
        shop = "西アドゥリン ?-?T? Ishvad",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [779] = {
        name = "インデフューリー",
        description = "自身の周囲のパーティメンバーの攻撃力をアップ。",
        shop = "西アドゥリン ?-?T? Ishvad",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [780] = {
        name = "インデバリア",
        description = "自身の周囲のパーティメンバーの防御力をアップ。",
        shop = "西アドゥリン ?-?T? Ishvad",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [781] = {
        name = "インデアキュメン",
        description = "自身の周囲のパーティメンバーの魔法攻撃力をアップ。",
        shop = "西アドゥリン ?-?T? Eukalline",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [782] = {
        name = "インデフェンド",
        description = "自身の周囲のパーティメンバーの魔法防御力をアップ。",
        shop = "西アドゥリン ?-?T? Ishvad",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [783] = {
        name = "インデプレサイス",
        description = "自身の周囲のパーティメンバーの命中率をアップ。(補足:遠隔攻撃も含む)",
        shop = "西アドゥリン ?-?T? Ishvad",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [784] = {
        name = "インデヴォイダンス",
        description = "自身の周囲のパーティメンバーの回避率をアップ。",
        shop = "西アドゥリン ?-?T? Ishvad",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [785] = {
        name = "インデフォーカス",
        description = "自身の周囲のパーティメンバーの魔法命中率をアップ。",
        shop = "西アドゥリン ?-?T? Ishvad",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [786] = {
        name = "インデアトゥーン",
        description = "自身の周囲のパーティメンバーの魔法回避率をアップ。",
        shop = "西アドゥリン ?-?T? Ishvad",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [787] = {
        name = "インデウィルト",
        description = "自身の周囲の敵の攻撃力をダウン。",
        shop = "西アドゥリン ?-?T? Eukalline",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [788] = {
        name = "インデフレイル",
        description = "自身の周囲の敵の防御力をダウン。",
        shop = "西アドゥリン ?-?T? Eukalline",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [789] = {
        name = "インデフェイド",
        description = "自身の周囲の敵の魔法攻撃力をダウン。",
        shop = "西アドゥリン ?-?T? Eukalline",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [790] = {
        name = "インデマレーズ",
        description = "自身の周囲の敵の魔法防御力をダウン。",
        shop = "西アドゥリン ?-?T? Eukalline",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [791] = {
        name = "インデスリップ",
        description = "自身の周囲の敵の命中率をダウン。",
        shop = "西アドゥリン ?-?T? Eukalline",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [792] = {
        name = "インデトーパー",
        description = "自身の周囲の敵の回避率をダウン。",
        shop = "西アドゥリン ?-?T? Eukalline",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [793] = {
        name = "インデヴェックス",
        description = "自身の周囲の敵の魔法命中率をダウン。",
        shop = "西アドゥリン ?-?T? Eukalline",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [794] = {
        name = "インデランゴール",
        description = "自身の周囲の敵の魔法回避率をダウン。",
        shop = "西アドゥリン ?-?T? Eukalline",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [795] = {
        name = "インデスロウ",
        description = "自身の周囲の敵の攻撃間隔を長くする。",
        shop = "西アドゥリン ?-?T? Eukalline",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [796] = {
        name = "インデパライズ",
        description = "自身の周囲の敵を麻痺させる。",
        shop = "西アドゥリン ?-?T? Eukalline",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [797] = {
        name = "インデグラビデ",
        description = "自身の周囲の敵をヘヴィ状態にして、移動速度を遅くする。",
        shop = "西アドゥリン ?-?T? Eukalline",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [771] = {
        name = "インデリジェネ",
        description = "自身の周囲のパーティメンバーのHPを徐々に回復。",
        shop = "西アドゥリン ?-?T? Eukalline",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [79] = {
        name = "スロウII",
        description = "敵の攻撃間隔を長くする。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [80] = {
        name = "パライズII",
        description = "敵を麻痺させる。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [107] = {
        name = "ファランクスII",
        description = "標的のパーティメンバーが受けるダメージを一定量軽減する。",
        shop = "ジュノ下層 H-9T9 Hasim",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "",
    },
    [478] = {
        name = "オーラ",
        description = "最大MPの20%消費する。標的のHPとMPを徐々に回復し、さらに攻撃間隔を短くする。",
        shop = "",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "学者レベル5以上でエルディーム古墳〔Ｓ〕 J-8T7 Erleneに話しかける",
    },
    [502] = {
        name = "メルトン",
        description = "最大MPの20%消費する。敵を闇属性のダメージがじわじわ蝕む。",
        shop = "",
        quest = "",
        drop = {},
        steal = {},
        chest = "",
        mining = "",
        content = "",
        acquisition = "学者レベル5以上でエルディーム古墳〔Ｓ〕 J-8T7 Erleneに話しかける",
    },
}

return magic_definitions